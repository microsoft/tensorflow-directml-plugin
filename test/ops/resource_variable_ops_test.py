...
class ResourceVariableOpsTest(test.TestCase, parameterized.TestCase):
    def tearDown(self):
        gc.collect()
    ...
    def testAssignVariableDtypeMismatchEager(self):
        with context.eager_mode():
            ...
            with self.assertRaisesRegex(
                errors.InvalidArgumentError,
                r"Trying to .* variable with wrong " r"dtype. Expected int32 got float",
            ):
                resource_variable_ops.assign_variable_op(
    ...
    def testScatterUpdateVariant(self):
        # TODO #37395305: Enable when we support TF_VARIANT
        self.skipTest("DML doesn't support the Variant datatype yet")
        with context.eager_mode():
            v = resource_variable_ops.ResourceVariable(
                [
