...
enum TensorFormat
{
    ...
    // activations for the same pixel into an int32_t. Thus an NCHW format
    // tensor with dimensions [N, C, H, W] would have dimensions [N, C/4, H, W,
    // 4] in NCHW_VECT_C format. A pre-condition of this format is that C must
    // be a multiple of 4.
...
enum FilterTensorFormat
{
    ...
    // int8 convolution and fused convolution. It is analogous to the
    // NCHW_VECT_C data format. It is laid out in the same order as OIHW, except
    // that the size of the Input Channels dimension is divided by 4, and a new
    // dimension of size 4 is appended, which packs 4 adjacent input channel
    // weights into an int32_t. Thus an OIHW format filter with dimensions [O,
    // I, H, W] would have dimensions [O, I/4, H, W, 4] in OIHW_VECT_I format. A
    // pre-condition of this format is that I must be a multiple of 4.
...
inline int GetTensorSpatialDims(int num_dims, TensorFormat format)
{
    ...
    case FORMAT_NCHW:
    case FORMAT_HWNC:
    case FORMAT_HWCN: return num_dims - 2; // Exclude N,C.
    ...
    default:
        TF_Log(TF_FATAL, "Unknown format %d", static_cast<int32_t>(format));
...
inline int GetTensorDimsFromSpatialDims(
    ...
    TensorFormat format)
{
    ...
    case FORMAT_NCHW:
    case FORMAT_HWNC:
    case FORMAT_HWCN: return num_spatial_dims + 2; // Include N,C.
    case FORMAT_NCHW_VECT_C:
    case FORMAT_NHWC_VECT_W:
...
inline int GetFilterTensorDimsFromSpatialDims(
    int num_spatial_dims,
    FilterTensorFormat format)
{
    if (format == FORMAT_OIHW_VECT_I)
...
inline int GetTensorBatchDimIndex(int num_dims, TensorFormat format)
{
    ...
    case FORMAT_NCHW:
    case FORMAT_NCHW_VECT_C:
    case FORMAT_NHWC_VECT_W: return 0;
    case FORMAT_HWNC: return num_dims - 2;
    case FORMAT_HWCN: return num_dims - 1;
    default:
        TF_Log(TF_FATAL, "Unknown format %d", static_cast<int32_t>(format));
        return -1; // Avoid compiler warning about missing return value
    }
...
inline int GetTensorFeatureDimIndex(int num_dims, TensorFormat format)
{
    ...
    {
    case FORMAT_NHWC:
    case FORMAT_HWNC: return num_dims - 1;
    case FORMAT_NHWC_VECT_W:
    case FORMAT_HWCN: return num_dims - 2;
    case FORMAT_NCHW:
...
{
    assert(format == FORMAT_NCHW_VECT_C);
    return num_dims - 1;
}
...
{
    assert(format == FORMAT_NHWC_VECT_W);
    return num_dims - 1;
}
    ...
    int spatial_dim)
{
    ...
    {
    case FORMAT_NHWC:
    case FORMAT_NHWC_VECT_W: return spatial_dim + 1;
    case FORMAT_NCHW:
    case FORMAT_NCHW_VECT_C: return spatial_dim + 2;
    case FORMAT_HWNC:
    case FORMAT_HWCN: return spatial_dim;
    default:
        TF_Log(TF_FATAL, "Unknown format %d", static_cast<int32_t>(format));
        return -1; // Avoid compiler warning about missing return value
    }
...
inline int GetFilterTensorSpatialDimIndex(
    int num_dims,
    FilterTensorFormat format,
    int dim)
{
    ...
    switch (format)
    {
    case FORMAT_HWIO: return dim;
    case FORMAT_OIHW:
    case FORMAT_OIHW_VECT_I: return dim + 2;
    default:
        TF_Log(TF_FATAL, "Unknown format %d", static_cast<int32_t>(format));
        return -1; // Avoid compiler warning about missing return value
    }
...
inline int GetFilterTensorInnerInputChannelsDimIndex(
    ...
    FilterTensorFormat format)
{
    assert(format == FORMAT_OIHW_VECT_I);
    return num_dims - 1;
}
    ...
    FilterTensorFormat format)
{
    switch (format)
    {
    case FORMAT_HWIO: return num_dims - 2;
    case FORMAT_OIHW:
    case FORMAT_OIHW_VECT_I: return 1;
    default:
        TF_Log(TF_FATAL, "Unknown format %d", static_cast<int32_t>(format));
        return -1; // Avoid compiler warning about missing return value
    }
...
inline int GetFilterTensorOutputChannelsDimIndex(
    int num_dims,
    FilterTensorFormat format)
{
    switch (format)
    {
    case FORMAT_HWIO: return num_dims - 1;
    case FORMAT_OIHW:
    case FORMAT_OIHW_VECT_I: return 0;
    default:
        TF_Log(TF_FATAL, "Unknown format %d", static_cast<int32_t>(format));
        return -1; // Avoid compiler warning about missing return value
    }
...
inline int32_t GetTensorDimIndex(TensorFormat format, char dimension)
{
    ...
    switch (dimension) {
      ...
      default:
        TF_Log(TF_FATAL, "Invalid dimension: %c", dimension);
        return -1;  // Avoid compiler warning about missing return value
    }
    ...
    TF_Log(TF_FATAL, "Invalid format: %d", static_cast<int>(format));
    return -1;  // Avoid compiler warning about missing return value
  }
    // clang-format on
}
    ...
    char dimension)
{
  ...
  if (filter_tensor_format == FORMAT_HWIO) {
    ...
    TF_Log(TF_FATAL, "Invalid format: %d", static_cast<int>(filter_tensor_format));
    return -1;  // Avoid compiler warning about missing return value
  }
    // clang-format on
}
...
inline int32_t GetTensorDimIndex(
    TensorFormat format,
    char dimension,
    int num_total_dims)
{
...
template <typename T>
T GetTensorDim(
    absl::Span<const T> dimension_attributes,
    TensorFormat tensor_format,
    char dimension)
{
    int index = GetTensorDimIndex(
        tensor_format,
        dimension,
        dimension_attributes.size());
...
template <typename T>
T GetFilterDim(
    absl::Span<const T> dimension_attribute,
    FilterTensorFormat filter_tensor_format,
    char dimension)
{
    int index = (GetFilterTensorSpatialDims(
                     dimension_attribute.size(),
    ...
    using size_type = typename absl::Span<const T>::size_type;
    CHECK(
        index >= 0 &&
...
T GetTensorDim(
    ...
    char dimension)
{
    return GetTensorDim(absl::Span<const T>(attributes), format, dimension);
}

// Return the size of the specified 'dimension' within 'tensor_shape'
// according to 'tensor_format'.
inline int64_t GetTensorDim(
    const TensorShape& tensor_shape,
    TensorFormat tensor_format,
    char dimension)
{
    return GetTensorDim(
        absl::Span<const int64_t>(tensor_shape.dim_sizes()),
        tensor_format,
        dimension);
}
...
inline TensorShape ShapeFromFormat(
    TensorFormat format,
    int64_t N,
    ...
    int64_t C)
{
    ...
    absl::InlinedVector<int64_t, 6> dim_sizes(dims);
    ...
    int64_t O)
{
    ...
    for (int dim = 0; static_cast<size_t>(dim) < spatial.size(); dim++)
    {
        dim_sizes[GetFilterTensorSpatialDimIndex(dims, format, dim)] =
            spatial[dim];
    }
...
inline TensorShape ShapeFromFilterFormat(
    FilterTensorFormat dst_filter_format,
    const TensorShape& src_shape,
