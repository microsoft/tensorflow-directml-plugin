...
Status GetPaddingFromString(absl::string_view str_value, Padding* value)
{
    if (str_value == "SAME")
    {
        *value = SAME;
    ...
    if (padding_type == Padding::EXPLICIT)
    {
        if (explicit_paddings.size() != 2 * num_dims)
        {
            return errors::InvalidArgument(
                "explicit_paddings attribute must contain ",
                2 * num_dims,
                " values, but got: ",
                explicit_paddings.size());
        }
        ...
        const int32_t batch_index =
            GetTensorBatchDimIndex(num_dims, data_format);
        const int32_t depth_index =
            GetTensorFeatureDimIndex(num_dims, data_format);
        ...
        {
            return errors::InvalidArgument("Nonzero explicit padding in the "
                                           "batch or depth dimensions is not "
                                           "supported");
        }
    }
    else if (!explicit_paddings.empty())
    {
        return errors::InvalidArgument("explicit_paddings attribute must be "
                                       "empty if the padding attribute is "
                                       "not EXPLICIT");
    }
    return Status::OK();
}
