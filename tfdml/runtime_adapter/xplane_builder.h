...
class XStatsBuilder
{
  public:
    ...
    // Adds a stat by copying a stat from another XPlane. Does not check if a
    // stat with the same metadata already exists in the event. To avoid
    // duplicated stats, use the variant below.
    void AddStat(
        const XStatMetadata& metadata,
        const XStat& src_stat,
  ...
  private:
    ...
    static void SetStatValue(unsigned long value, XStat* stat)
    { // NOLINT
        stat->set_uint64_value(value);
    }
    static void SetStatValue(unsigned long long value, XStat* stat)
    { // NOLINT
        stat->set_uint64_value(value);
    }
    ...
    static void SetStatValue(long value, XStat* stat)
    { // NOLINT
        stat->set_int64_value(value);
    }
    static void SetStatValue(long long value, XStat* stat)
    { // NOLINT
        stat->set_int64_value(value);
    }
        ...
        XStat* dst_stat)
    {
        ...
        {
        case XStat::VALUE_NOT_SET: break;
...
class XEventBuilder : public XStatsBuilder<XEvent>
{
  public:
    XEventBuilder(const XLine* line, XPlaneBuilder* plane, XEvent* event)
        : XStatsBuilder<XEvent>(event, plane),
          line_(line),
          event_(event)
    {
    }
...
class XPlaneBuilder : public XStatsBuilder<XPlane>
{
  public:
    ...
    // Returns a builder for the line with the given id. Creates a new line if
    // the id was unused, otherwise the builder will add events to an existing
    // line.
    ...
    // Returns event metadata with the given id. Creates a new metadata if the
    // id was unused. WARNING: If calling this function, don't call the string
    // overloads below on the same instance.
    XEventMetadata* GetOrCreateEventMetadata(int64_t metadata_id);

    // Returns event metadata with the given name. The id is internally
    // assigned. Creates a new metadata if the name was unused. Using these
    // overloads guarantees names are unique. WARNING: If calling any of these
    // overloads, do not call the integer one above on the same instance.
