...
class PhiloxRandom
{
  public:
    ...
    inline explicit PhiloxRandom(uint64_t seed)
    {
        key_[0] = static_cast<uint32_t>(seed);
        key_[1] = static_cast<uint32_t>(seed >> 32);
    }

    inline explicit PhiloxRandom(uint64_t seed_lo, uint64_t seed_hi)
    {
    ...
    inline PhiloxRandom(ResultType counter, Key key)
        : counter_(counter),
          key_(key)
    {
    }

    inline ResultType const& counter() const { return counter_; }

    inline Key const& key() const { return key_; }

    // Skip the specified number of samples of 128-bits in the current stream.
    inline void Skip(uint64_t count)
    {
  ...
  private:
    ...
    // Helper function to skip the next sample of 128-bits in the current
    // stream.
    inline void SkipOne()
    {
        if (++counter_[0] == 0)
    ...
    inline static void MultiplyHighLow(
        uint32_t a,
        uint32_t b,
        uint32_t* result_low,
