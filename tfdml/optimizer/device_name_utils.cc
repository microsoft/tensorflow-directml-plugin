...
static bool ConsumePrefix(
    absl::string_view* in,
    std::string* out,
    absl::string_view prefix_terminators)
{
    if (in->empty() || !IsAlpha(in->front())) return false;
    const auto end_it = std::find_first_of(
        in->begin(),
        in->end(),
        prefix_terminators.begin(),
...
{
    uint64_t tmp;
    if (ConsumeLeadingDigits(in, &tmp))
    {
        *val = tmp;
        return true;
    }
...
/* static */
std::string DeviceNameUtils::FullName(
    const std::string& job,
    int replica,
    int task,
...
bool DeviceNameUtils::ParseFullOrLocalName(
    absl::string_view fullname,
    ParsedName* p)
{
...
void CompleteName(
    const DeviceNameUtils::ParsedName& parsed_basename,
...
/* static */
Status DeviceNameUtils::CanonicalizeDeviceName(
    absl::string_view fullname,
...
{
    std::string buf;
    if (pn.has_job) absl::StrAppend(&buf, "/job:", pn.job);
    if (pn.has_replica) absl::StrAppend(&buf, "/replica:", pn.replica);
    if (pn.has_task) absl::StrAppend(&buf, "/task:", pn.task);
    if (pn.has_type)
    {
...
/* static */
bool DeviceNameUtils::IsSpecification(
...
/* static */
bool DeviceNameUtils::AreCompatibleDevNames(
    const ParsedName& a,
    const ParsedName& b)
{
...
void DeviceNameUtils::EnsureSpecification(
    ParsedName* more_specific,
    const ParsedName& less_specific)
{
    if (less_specific.has_job)
    {
...
/* static */
bool DeviceNameUtils::IsCompleteSpecification(
    const ParsedName& pattern,
    const ParsedName& name)
{
    CHECK(
        name.has_job && name.has_replica && name.has_task && name.has_type &&
        name.has_id);
...
/* static */
Status DeviceNameUtils::MergeDevNames(
    ParsedName* target,
    const ParsedName& other,
    bool allow_soft_placement)
{
    return MergeDevNamesImpl(
        target,
        other,
...
/* static */
Status DeviceNameUtils::MergeOverrideDevNames(
    ParsedName* target,
    const ParsedName& other)
{
    return MergeDevNamesImpl(
...
/* static */
void DeviceNameUtils::MergeUnsetDevNames(
    ParsedName* target,
    const ParsedName& other)
{
...
/* static */
bool DeviceNameUtils::IsSameAddressSpace(
    const ParsedName& a,
    const ParsedName& b)
{
...
/* static */
bool DeviceNameUtils::IsDifferentAddressSpace(
    const ParsedName& a,
    const ParsedName& b)
{
    return (a.has_job && b.has_job && (a.job != b.job)) ||
...
{
    ParsedName x;
    CHECK(ParseFullName(fullname, &x));
...
/* static */
bool DeviceNameUtils::SplitDeviceName(
    ...
    std::string* device)
{
    ...
    if (ParseFullName(name, &pn) && pn.has_type && pn.has_id)
    {
        ...
        task->reserve(
            ...
            (pn.has_replica ? (9 + 4 /*estimated UB for # replica digits*/)
                            : 0) +
    ...
    const ParsedName& pn)
{
    ...
    {
        return {
            DeviceNameUtils::LocalName(pn.type, pn.id),
