...
{
  public:
    NodeDefAndPortIndex(const tensorflow::NodeDef* node_def, int port_index)
        : node_def_(node_def),
          port_index_(port_index)
    {
    }
...
template <
    typename FaninViewT,
    typename FanoutViewT,
    typename GraphViewT,
    bool IsConst>
class NodeViewInternal
{
  private:
    using NodeDefT = typename std::conditional<
        IsConst,
        const tensorflow::NodeDef,
        tensorflow::NodeDef>::type;

  public:
    explicit NodeViewInternal(GraphViewT* graph_view, int node_index)
        : graph_view_(graph_view),
          node_index_(node_index),
          attrs_(&graph_view->graph()->node(node_index).attr())
    {
    }
    ...
    const std::string& GetDevice() const { return node()->device(); }
    ...
    // Returns a regular fanout(s) based on output index. If no such output
    // index exists, no fanouts will be returned.
    ...
    int NumAttrs() const { return attrs_ ? attrs_->size() : 0; }
  ...
  protected:
    ...
    GraphViewT* graph_view_;
    const google::protobuf::Map<std::string, tensorflow::AttrValue>* attrs_;
    int node_index_;
};
...
{
  private:
    using GraphDefT = typename std::conditional<
        IsConst,
        const tensorflow::GraphDef,
        tensorflow::GraphDef>::type;
...
inline SafeTensorId EmptyTensorId() { return SafeTensorId("", kMissingSlot); }

inline bool IsEmptyTensorId(const TensorId tensor_id)
{
    return tensor_id.node().empty() && tensor_id.index() == kMissingSlot;
}
...
struct NodeViewDiff
{
    ...
    absl::flat_hash_map<std::string, tensorflow::AttrValue> attrs_to_add;
    absl::flat_hash_set<std::string> attrs_to_remove;
    // google::protobuf::Map<std::string, tensorflow::AttrValue> constructor and
    // destructor are very expensive, we will initialize it lazily only if
    // needed.
    absl::optional<google::protobuf::Map<std::string, tensorflow::AttrValue>>
        processed_attrs;
};
...
template <typename GraphViewT>
inline bool AddOrUpdateRegularFanin(
    NodeViewDiff<GraphViewT>* diff,
    int index,
    const TensorId& fanin)
{
    ...
    { // Updating existing fanins.
            ...
            existing_fanin.node_view()->GetName() != fanin.node())
        {
            ...
            diff->regular_inputs_to_update[index] = SafeTensorId(fanin);
        }
    }
    else
    {
        ...
        if (AddOrUpdateAtIndex(
                &diff->regular_inputs_to_add,
                relative_add_index,
                fanin,
                EmptyTensorId()))
        {
...
inline bool RemoveRegularFanin(NodeViewDiff<GraphViewT>* diff, int index)
{
    ...
    else
    {
        ...
        const int relative_add_index = index - num_regular_fanins;
        int diff_regular_inputs_to_add_size =
...
template <typename GraphViewT>
inline bool AddControllingFanin(
    NodeViewDiff<GraphViewT>* diff,
    int control_index,
...
template <typename GraphViewT>
inline bool RemoveControllingFanin(
    NodeViewDiff<GraphViewT>* diff,
    int control_index,
    absl::string_view fanin_node_name)
{
    ...
    const absl::flat_hash_map<absl::string_view, int>& updated_node_names)
{
    ...
    if (diff_regular_inputs_to_add_size != diff->num_regular_inputs_to_add)
    {
        ...
        diff->num_regular_inputs_to_add > 0 &&
        !diff->regular_inputs_to_remove.empty())
    {
        // Appending new fanins while removing existing fanins, resulting in
        // missing regular fanins in between.
        return false;
    }
    else if (
        static_cast<int>(diff->regular_inputs_to_remove.size()) !=
        diff->num_regular_inputs_to_remove)
    {
    ...
    {
        return fanin_node_name == node_name || !CheckNodeNameExists(
                                                   fanin_node_name,
                                                   updated_node_names,
                                                   diff->graph_view);
    };

    // Check if nodes of all updated and new fanins exist (from name) and if
    // such fanins do not introduce self loops. Note, this will not check for if
    // unmodified fanins exist.
    if (diff->update_name)
    {
        // If name of node was changed in node, check all fanins. Updated fanins
        // are checked for existence and self loops. Unmodified fanins are
        // checked for self loops. `regular_inputs_to_update`,
        // `controlling_inputs_to_remove` are sorted, so iterators from these
        // maps/sets can be incremented alongside iteration and be used for
        // comparisons.
        const int last_index = node_view->NumRegularFanins() -
    ...
    else
    {
        ...
        {
            const std::string& fanin_name = updated.second.node();
...
struct NewNode
{
    explicit NewNode(GraphViewT* graph_view, tensorflow::NodeDef&& node)
        : graph_view(graph_view),
          node(std::move(node))
    {
    }
...
inline void UpdateName(NewNode<GraphViewT>* new_node, absl::string_view name)
{
    ...
    {
        new_node->node.set_name(std::string(name));
    }
}
    ...
    absl::string_view device)
{
    ...
    {
        new_node->node.set_device(std::string(device));
    }
}
    ...
    const TensorId& fanin)
{
    ...
    else if (AddOrUpdateAtIndex(
                 &new_node->regular_fanins,
                 index,
                 fanin,
                 EmptyTensorId()))
    {
...
template <typename GraphViewT>
inline void AddControllingFanin(
    NewNode<GraphViewT>* new_node,
    absl::string_view fanin_node_name)
{
...
template <typename GraphViewT>
inline void RemoveControllingFanin(
    NewNode<GraphViewT>* new_node,
    absl::string_view fanin_node_name)
{
...
template <typename GraphViewT>
inline void AddOrUpdateAttribute(
    NewNode<GraphViewT>* new_node,
    absl::string_view attr_name,
    const tensorflow::AttrValue& attr_value)
{
    auto iter = new_node->node.mutable_attr()->find(std::string(attr_name));
...
inline void RemoveAttribute(
    ...
    absl::string_view attr_name)
{
    new_node->node.mutable_attr()->erase(std::string(attr_name));
}
    ...
    const absl::flat_hash_map<absl::string_view, int>& updated_node_names)
{
    ...
    auto invalid_node_name = [new_node, updated_node_names, node_name](
                                 absl::string_view fanin_node_name)
    {
        return fanin_node_name == node_name || !CheckNodeNameExists(
                                                   fanin_node_name,
