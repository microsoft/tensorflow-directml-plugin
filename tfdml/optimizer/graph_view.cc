...
static void PermuteNodesInPlace(
    tensorflow::GraphDef* graph,
    std::vector<int>* permutation,
    bool invert_permutation)
{
    CHECK(graph->node_size() == permutation->size());
...
FaninView::FaninView(NodeView* node_view, int index)
    : NodeIndexAndPortIndex(
          node_view->graph_view_,
          node_view->node_index_,
          index)
{
}
...
const tensorflow::NodeDef* NodeView::node() const
{
...
{
    if (fanin.index() < kControlSlot || graph_view_ != fanin.graph_view_)
    {
        return false;
    }
...
{
    if (fanout.index() < kControlSlot || graph_view_ != fanout.graph_view_)
    {
        return false;
    }
    ...
    else if (fanout.index() == kControlSlot)
    {
        return view->fanins_set_.contains({this->node(), kControlSlot});
    }
...
GraphView::GraphView(const tensorflow::GraphDef* graph, Status* status)
    ...
    for (const tensorflow::NodeDef& node : graph->node())
    {
        if (!AddUniqueNodeInternal(&node))
        {
            *status = errors::InvalidArgument(
                kGraphViewError,
                "graph has multiple nodes with the name '",
                node.name(),
                "'.");
            Reset();
            return;
...
bool GraphView::AddUniqueNodeInternal(const tensorflow::NodeDef* node)
{
...
Status GraphView::CheckAndAddFaninsInternal(NodeView* node_view)
{
    ...
    const tensorflow::NodeDef* node = node_view->node();
    ...
    for (const std::string& input : node->input())
    {
        ...
        if (is_control)
        {
            ...
            node_view->controlling_fanins_.emplace_back(
                ...
                fanin_node_index,
                kControlSlot);
            node_view->fanins_set_.emplace(
                fanin_node_view.node(),
                kControlSlot);
        ...
        else
        {
            ...
            if (fanin_node_view_regular_fanouts_by_port_size <
                fanin_id.index() + 1)
            {
                fanin_node_view.regular_fanouts_by_port_.resize(
                    fanin_id.index() + 1);
            }
            fanin_node_view.regular_fanouts_by_port_[fanin_id.index()]
                .emplace_back(
                    this,
                    node_index,
                    static_cast<int>(node_view->regular_fanins_.size()));
...
tensorflow::NodeDef* MutableNodeView::node() const
{
...
bool MutableNodeView::HasFanout(const MutableFaninView& fanout) const
{
    ...
    {
        return view->fanins_count_.contains({this->node(), kControlSlot});
    }
    else if (fanout.index() >= static_cast<int>(view->regular_fanins_.size()))
    {
        return false;
...
MutationNewNode Mutation::AddNode(tensorflow::NodeDef&& node, Status* status)
{
    bool has_observed_control = false;
    const std::string& node_name = node.name();
    ...
    for (int i = 0; i < num_fanins; ++i)
    {
        ...
        {
            *status = errors::InvalidArgument(
                kMutationAddNodeError,
                "node '",
                node_name,
                "' has self cycle fanin '",
                input,
                "'.");
            return MutationNewNode(this, mutation_counter_, kMissingIndex);
        }
        bool is_control = IsTensorIdControl(fanin_id);
        if (is_control)
        {
...
{
    assert(node->graph_view_ == graph_view_);
    if (node->update_index_ == kMissingIndex)
    {
        MutableNodeViewDiff diff(graph_view_, node->node_index_);
        // If mutation is a no-op return and do not add it to the
        // `updated_nodes_`.
...
void Mutation::UpdateNodeName(MutableNodeView* node, absl::string_view name)
{
    AddMutation(
        node,
        [name](MutableNodeViewDiff* diff) { return UpdateName(diff, name); });
}

void Mutation::UpdateNodeName(
    const MutationNewNode& node,
    absl::string_view name)
{
    assert(
        node.mutation_ == this && node.mutation_counter_ == mutation_counter_);
    UpdateName(&new_nodes_[node.index_], name);
}
...
void Mutation::UpdateNodeOp(MutableNodeView* node, absl::string_view op)
{
    AddMutation(
        node,
        [op](MutableNodeViewDiff* diff) { return UpdateOp(diff, op); });
}
...
void Mutation::UpdateNodeOp(const MutationNewNode& node, absl::string_view op)
{
    assert(
        node.mutation_ == this && node.mutation_counter_ == mutation_counter_);
    UpdateOp(&new_nodes_[node.index_], op);
}

void Mutation::UpdateNodeDevice(MutableNodeView* node, absl::string_view device)
{
    AddMutation(
        node,
        [device](MutableNodeViewDiff* diff)
        { return UpdateDevice(diff, device); });
}

void Mutation::UpdateNodeDevice(
    const MutationNewNode& node,
    absl::string_view device)
{
    assert(
        node.mutation_ == this && node.mutation_counter_ == mutation_counter_);
    UpdateDevice(&new_nodes_[node.index_], device);
}

void Mutation::AddOrUpdateRegularFanin(
    MutableNodeView* node,
    int index,
    const TensorId& fanin)
{
    AddMutation(
        node,
        [index, fanin](MutableNodeViewDiff* diff)
        { return tfdml::AddOrUpdateRegularFanin(diff, index, fanin); });
}
    ...
    const TensorId& fanin)
{
    assert(
        node.mutation_ == this && node.mutation_counter_ == mutation_counter_ &&
        index >= 0 && IsTensorIdRegular(fanin));
    tfdml::AddOrUpdateRegularFanin(&new_nodes_[node.index_], index, fanin);
}
...
void Mutation::RemoveRegularFanin(MutableNodeView* node, int index)
{
    AddMutation(
        node,
        [index](MutableNodeViewDiff* diff)
        { return tfdml::RemoveRegularFanin(diff, index); });
}
...
void Mutation::RemoveRegularFanin(const MutationNewNode& node, int index)
{
    assert(
        node.mutation_ == this && node.mutation_counter_ == mutation_counter_ &&
        index >= 0);
    tfdml::RemoveRegularFanin(&new_nodes_[node.index_], index);
}

void Mutation::AddControllingFanin(
    MutableNodeView* node,
    absl::string_view fanin_node_name)
{
    AddMutation(
        node,
        [node, fanin_node_name](MutableNodeViewDiff* diff)
        {
            auto it = node->controlling_fanins_index_.find(fanin_node_name);
            const int control_index =
                it != node->controlling_fanins_index_.end() ? it->second
                                                            : kMissingIndex;
            return tfdml::AddControllingFanin(
                diff,
                control_index,
                fanin_node_name);
        });
    ...
    absl::string_view fanin_node_name)
{
    assert(
        node.mutation_ == this && node.mutation_counter_ == mutation_counter_);
    tfdml::AddControllingFanin(&new_nodes_[node.index_], fanin_node_name);
}

void Mutation::RemoveControllingFanin(
    MutableNodeView* node,
    absl::string_view fanin_node_name)
{
    AddMutation(
        ...
        [node, fanin_node_name](MutableNodeViewDiff* diff)
        {
            ...
            const int control_index =
                it != node->controlling_fanins_index_.end() ? it->second
                                                            : kMissingIndex;
            return tfdml::RemoveControllingFanin(
                diff,
                control_index,
                fanin_node_name);
        });
    ...
    absl::string_view fanin_node_name)
{
    assert(
        node.mutation_ == this && node.mutation_counter_ == mutation_counter_);
    tfdml::RemoveControllingFanin(&new_nodes_[node.index_], fanin_node_name);
}

void Mutation::AddOrUpdateNodeAttr(
    MutableNodeView* node,
    ...
    const tensorflow::AttrValue& attr_value)
{
    AddMutation(
        node,
        [attr_name, attr_value](MutableNodeViewDiff* diff)
        { return AddOrUpdateAttribute(diff, attr_name, attr_value); });
}
    ...
    const tensorflow::AttrValue& attr_value)
{
    assert(
        node.mutation_ == this && node.mutation_counter_ == mutation_counter_);
    AddOrUpdateAttribute(&new_nodes_[node.index_], attr_name, attr_value);
}

void Mutation::RemoveNodeAttr(
    MutableNodeView* node,
    absl::string_view attr_name)
{
    AddMutation(
        node,
        [attr_name](MutableNodeViewDiff* diff)
        { return RemoveAttribute(diff, attr_name); });
}
    ...
    absl::string_view attr_name)
{
    assert(
        node.mutation_ == this && node.mutation_counter_ == mutation_counter_);
    RemoveAttribute(&new_nodes_[node.index_], attr_name);
}
...
void Mutation::Reset()
{
    ...
    {
        graph_view_->nodes_[update.node_index].update_index_ = kMissingIndex;
    }
    ResetInternal();
}
...
inline void IncrementFaninCount(
    absl::flat_hash_map<NodeDefAndPortIndex, int>* fanins_count,
    const NodeDefAndPortIndex& fanin)
{
    ++(*fanins_count)[fanin];
}
...
MutableGraphView::MutableGraphView(tensorflow::GraphDef* graph, Status* status)
    : GraphViewInternal(graph),
      mutation_(Mutation(this))
{
    const int num_nodes = graph->node_size();
    node_index_by_name_.reserve(num_nodes);
    nodes_.reserve(num_nodes);
    for (tensorflow::NodeDef& node : *graph->mutable_node())
    {
        if (!AddUniqueNodeInternal(&node))
        {
            *status = errors::InvalidArgument(
                kMutableGraphViewError,
                "graph has multiple nodes with the name '",
                node.name(),
                "'.");
            Reset();
...
bool MutableGraphView::AddUniqueNodeInternal(tensorflow::NodeDef* node)
{
    const int node_index = node_index_by_name_.size();
    auto it = node_index_by_name_.emplace(node->name(), node_index);
    if (it.second)
    ...
    std::vector<std::vector<TensorId>>* fanins)
{
    ...
    {
        bool has_observed_control = false;
        const tensorflow::NodeDef* node = nodes_[i].node();
        const std::string& node_name = node->name();
    ...
    std::vector<std::vector<TensorId>>* fanins)
{
    ...
    for (int i = 0; i < num_nodes; ++i)
    {
        ...
        tensorflow::NodeDef* node = node_view.node();
        ...
        while (pos <= last_pos)
        {
            ...
            if (is_control)
            {
                if (observed_controls.insert(fanin_id.node()).second)
                {
                    fanin_node_view.controlled_fanouts_.emplace_back(
                        this,
                        i,
                        kControlSlot,
                        static_cast<int>(node_view.controlling_fanins_.size()));
                    node_view.controlling_fanins_.emplace_back(
                        this,
                        fanin_node_index,
                        kControlSlot,
                        static_cast<int>(
                            fanin_node_view.controlled_fanouts_.size()) -
                            1);
                    IncrementFaninCount(
                        &node_view.fanins_count_,
                        {&graph_->node(fanin_node_index), kControlSlot});
                    node_view.controlling_fanins_index_.emplace(
                        fanin_id.node(),
                        pos - node_view.NumRegularFanins());
                    ++pos;
                }
                else
                {
            ...
            else
            {
                ...
                auto& fanin_regular_fanouts =
                    fanin_node_view.regular_fanouts_by_port_[fanin_id.index()];
                fanin_regular_fanouts.emplace_back(
                    this,
                    i,
                    static_cast<int>(node_view.regular_fanins_.size()),
                    static_cast<int>(node_view.regular_fanins_.size()));
                ...
                node_view.regular_fanins_.emplace_back(
                    ...
                    fanin_node_index,
                    fanin_id.index(),
                    static_cast<int>(fanin_regular_fanouts.size()) - 1);
                IncrementFaninCount(
                    &node_view.fanins_count_,
        ...
        {
            node->mutable_input()->DeleteSubrange(
                last_pos + 1,
                last_idx - last_pos);
        }
    }
    ...
    std::vector<int>* empty_diff_node_indices)
{
    ...
    {
        return errors::InvalidArgument(
            kMutableGraphViewApplyError,
            "multiple nodes with the name: '",
            node_name,
            "' exists in Mutation.");
    };
    ...
    for (int i = 0; i < num_updated_nodes; ++i)
    {
        ...
        auto it = node_names->insert({node_name, kNodeNamePresent});
        if (!it.second)
        {
            if (it.first->second == kNodeNamePresent)
            {
                // Another node in the mutation is already using this name,
                // which will result in a conflict.
                return name_conflict(node_name);
            }
            else
            {
                // Mark name as present (node was marked missing from either
                // being removed or renamed).
                it.first->second = kNodeNamePresent;
            }
        }
        if (diff.update_name)
        {
            // Lookup new name of node in current graph. If a node has such
            // name, store its index for later lookups as this node will be
            // overwritten.
    ...
    for (const auto& new_node : mutation_.new_nodes_)
    {
        ...
        if (it.first->second == kNodeNamePresent)
        {
            // Another node in the mutation is already using this name, which
...
{
    auto bad_fanout =
        [](absl::string_view fanout_node_name, absl::string_view node_name)
    {
        return errors::InvalidArgument(
            kMutableGraphViewApplyError,
            "fanout '",
            fanout_node_name,
            "' exist for missing node '",
            node_name,
            "'.");
    };
    ...
    {
        if (renamed_node.overwritten_node_index_ == kMissingIndex)
        {
            continue;
        }
    ...
    // Check if removed nodes and previous state of renamed nodes have no
    // fanouts.
    for (const auto& node_name_state : node_names)
    {
        if (node_name_state.second == kNodeNamePresent)
        {
            continue;
        }
        ...
        for (const auto& regular_fanouts : node_view.GetRegularFanouts())
        {
            for (const auto& regular_fanout : regular_fanouts)
            {
                ...
                if (fanout_view->update_index_ == kMissingIndex)
                {
                    if (mutation_.removed_nodes_.contains(
                            fanout_view->node_index_))
                    {
                    ...
                    else if (!overwritten_nodes[fanout_view->node_index_])
                    {
                        ...
                        return bad_fanout(
                            fanout_view->GetName(),
                            node_name_state.first);
                    }
                }
                else
                {
                    auto& diff =
                        mutation_.updated_nodes_[fanout_view->update_index_];
                    ...
                    if (diff.regular_inputs_to_update.find(
    ...
    const std::vector<int>& inplace_nodes)
{
    ...
    for (auto& inplace_node : inplace_nodes)
    {
        ...
        if (!IsWellFormed(&diff, node_names))
        {
            return errors::InvalidArgument(
                kMutableGraphViewApplyError,
                "inplace updated node '",
                nodes_[diff.node_index].GetName(),
                "' is ill-formed.");
        }
    }
    for (auto& renamed_node : renamed_nodes)
    {
        auto& diff =
            mutation_.updated_nodes_[renamed_node.renamed_update_index_];
        if (!IsWellFormed(&diff, node_names))
        {
            return errors::InvalidArgument(
                kMutableGraphViewApplyError,
                "renamed updated node '",
                diff.name,
                "' ('",
                nodes_[diff.node_index].GetName(),
                "') is ill-formed.");
    ...
    {
        if (!IsWellFormed(&new_node, node_names))
        {
            return errors::InvalidArgument(
                kMutableGraphViewApplyError,
                "new node '",
...
Status MutableGraphView::CheckKernelRegisteredForNodes()
{
    ...
    for (auto& diff : mutation_.updated_nodes_)
    {
        ...
        tensorflow::NodeDef* node = nodes_[diff.node_index].node();
        ...
        {
            auto iter = diff.processed_attrs->find(attr_to_add.first);
            ...
            {
                iter->second = attr_to_add.second;
            }
        }
...
void MutableGraphView::ReplaceNodeFanouts(MutableNodeView* node, T* fanouts)
{
    ...
    node->regular_fanouts_by_port_ =
        std::move(fanouts->regular_fanouts_by_port_);
    for (int i = 0, i_max = node->regular_fanouts_by_port_.size(); i < i_max;
         ++i)
    {
        for (int j = 0, j_max = node->regular_fanouts_by_port_[i].size();
             j < j_max;
             ++j)
        {
            ...
            auto& fanout_fanin =
                fanout_node_view->regular_fanins_[fanout.index()];
            auto* fanout_fanins_count = &fanout_node_view->fanins_count_;
            DecrementFaninCount(
                fanout_fanins_count,
                {&graph_->node(fanout_fanin.node_index_),
                 fanout_fanin.index()});
    ...
    for (int i = 0, i_max = node->controlled_fanouts_.size(); i < i_max; ++i)
    {
        ...
        DecrementFaninCount(
            fanout_fanins_count,
            {&graph_->node(fanout_fanin.node_index_), kControlSlot});
        fanout_fanin.node_index_ = node->node_index_;
        fanout_fanin.fanout_index_ = i;
        IncrementFaninCount(
            fanout_fanins_count,
            {&graph_->node(node->node_index_), kControlSlot});
    }
}
    ...
    std::vector<bool>* overwritten_name_removed_nodes)
{
    ...
    for (auto& renamed : *renamed_nodes)
    {
        ...
        if (fanouts_it != renamed_fanouts->end())
        {
            ...
            // Node to be overwritten is being renamed, so it won't be
            // overwritten.
            renamed.overwritten_node_index_ = kMissingIndex;
        }
        else if (renamed.overwritten_node_index_ != kMissingIndex)
        {
            ...
            if (mutation_.removed_nodes_.contains(
                    node_to_overwrite.node_index_))
            {
                (*overwritten_name_removed_nodes)[node_to_overwrite
                                                      .node_index_] = true;
            }
        }
        else
    ...
    const absl::flat_hash_map<std::string, NodeViewFanouts>& renamed_fanouts)
{
    ...
    // kMissingIndex as an indicator so when it comes to updating or
    ...
    for (auto& renamed_fanout : renamed_fanouts)
    {
             ...
             renamed_fanout.second.regular_fanouts_by_port_)
        {
            for (auto& fanout : regular_fanouts)
            {
                ...
                fanin.fanout_index_ = kMissingIndex;
            }
        }
        for (auto& fanout : renamed_fanout.second.controlled_fanouts_)
        {
            ...
            auto& fanin =
                fanout_node_view->controlling_fanins_[fanout.fanin_index_];
            fanout_node_view->fanins_count_.erase(
                {fanin.node_view()->node(), kControlSlot});
            fanout_node_view->controlling_fanins_index_.erase(
                renamed_fanout.first);
            fanin.fanout_index_ = kMissingIndex;
        }
    }
}
    ...
    int i)
{
    ...
    if (fanin.fanout_index_ == kMissingIndex)
    {
        return;
    }
    ...
    {
        // Swap fanout with last fanout in vector, and update it's associated
        // fanin index.
    ...
    // necessary to loop through all of the output ports to determine the
    // maximum output port before resizing.
    ...
    const SafeTensorId& fanin_id)
{
    ...
    node_view->regular_fanins_.emplace_back(
        this,
        fanin_node_view->node_index(),
        fanin_id.index(),
        static_cast<int>(fanouts.size()) - 1);
    IncrementFaninCount(
        &node_view->fanins_count_,
    ...
    const SafeTensorId& fanin_id)
{
    ...
    node_view->regular_fanins_[i] = MutableFanoutView(
        this,
        fanin_node_view->node_index(),
        fanin_id.index(),
        fanouts.size() - 1);
    ...
    int i)
{
    ...
    if (control_to_remove.fanout_index_ != kMissingIndex)
    {
        // Update internal state associated with node.
        node_view->fanins_count_.erase(
            {control_to_remove.node_view()->node(), kControlSlot});
            ...
            control_to_remove_view_controlled_fanouts_size - 1)
        {
            ...
            std::swap(
                control_to_remove_view_last_control,
    ...
    const std::set<int>& indices_to_remove)
{
         ...
         ++rit)
    {
        ...
        // Swap last controlling fanin in node with controlling fanin to be
        // removed.
        ...
        if (control_index < node_view_controlling_fanins_size - 1)
        {
            ...
            node_view->controlling_fanins_index_
                .find(last_control_view->GetName())
            ...
            std::swap(
                last_control,
                node_view->controlling_fanins_[control_index]);
        }
...
{
    tensorflow::NodeDef* node = node_view->node();
    // Add controlling fanin to tensorflow::NodeDef.
    ...
    fanin_node_view->controlled_fanouts_
        .emplace_back(this, node_view->node_index(), kControlSlot, index);
    node_view->controlling_fanins_.emplace_back(
        this,
        fanin_node_view->node_index(),
        kControlSlot,
        static_cast<int>(fanin_node_view->controlled_fanouts_.size()) - 1);
    IncrementFaninCount(
        &node_view->fanins_count_,
        {&graph_->node(fanin_node_view->node_index()), kControlSlot});
    // Parse new fanin std::string for node name.
...
void MutableGraphView::ApplyNodeUpdates()
{
    for (auto& diff : mutation_.updated_nodes_)
    {
        ...
        diff.node_index = kMissingIndex;
        // Clean up node view.
        node_view.update_index_ = kMissingIndex;

        tensorflow::NodeDef* node_def = node_view.node();
        ...
        if (diff.num_regular_inputs_to_remove > 0)
        {
            ...
            const int first_index = node_view.NumRegularFanins() -
                                    diff.num_regular_inputs_to_remove;
            ...
            node_def->mutable_input()->DeleteSubrange(
                node_view.NumRegularFanins(),
                diff.num_regular_inputs_to_remove);
        }
        else if (diff.num_regular_inputs_to_add > 0)
        {
            // Append regular fanins.
            node_def->mutable_input()->Reserve(
                node_def->mutable_input()->size() +
            ...
            for (const SafeTensorId& fanin : diff.regular_inputs_to_add)
            {
                ...
                node_def->mutable_input()->SwapElements(
                    curr_index,
            ...
            // `node_view.controlling_fanins_` as `num_regular_inputs_to_add_`
            // may not be a multiple of `num_regular_inputs_to_add_`. This is to
            // prevent rehashing controlling fanins in
            // `node_view.controlling_fanins_index_`.
            if (node_view.NumControllingFanins() > 1 &&
                curr_control_start != node_view.NumRegularFanins())
            {
                std::rotate(
                    node_def->mutable_input()->begin() +
                        node_view.NumRegularFanins(),
        ...
        RemoveControllingFaninInternal(
            &node_view,
    ...
    const std::vector<int>& new_node_indices)
{
    ...
    for (const int new_node_index : new_node_indices)
    {
        ...
        tensorflow::NodeDef* new_node_def = new_node_view.node();
        new_node_def->mutable_input()->Reserve(
            new_node->num_regular_fanins + new_node->controlling_fanins.size());
    ...
    const std::vector<bool>& overwritten_name_removed_nodes)
{
    ...
    {
        if (renamed.overwritten_node_index_ != kMissingIndex)
        {
    ...
    node_indices_to_remove.reserve(
        mutation_.updated_nodes_.size() + overwritten_nodes.size());
    for (int node_index : mutation_.removed_nodes_)
    {
    ...
    node_indices_to_remove.insert(
        node_indices_to_remove.end(),
        overwritten_nodes.begin(),
        overwritten_nodes.end());
    std::set<int> sorted_node_indices_to_remove(
        node_indices_to_remove.begin(),
        node_indices_to_remove.end());

    // Iterate in descending order so indices stay consistent.
    for (auto rit = sorted_node_indices_to_remove.rbegin();
         rit != sorted_node_indices_to_remove.rend();
         ++rit)
    {
        ...
        if (last_node.node_index_ > removed_node_index)
        {
            ...
            for (auto& regular_fanouts : last_node.regular_fanouts_by_port_)
            {
                for (auto& regular_fanout : regular_fanouts)
                {
                    ...
                    MutableNodeView* fanout_node_view =
                        regular_fanout.node_view();
                    fanout_node_view
                        ->regular_fanins_[regular_fanout.fanin_index_]
                        .node_index_ = removed_node_index;
                }
            }
            for (auto& controlled_fanout : last_node.controlled_fanouts_)
            {
                ...
                MutableNodeView* fanout_node_view =
                    controlled_fanout.node_view();
                fanout_node_view
                    ->controlling_fanins_[controlled_fanout.fanin_index_]
                    .node_index_ = removed_node_index;
            }

            const int last_node_index = nodes_.size() - 1;
            std::swap(nodes_[last_node_index], nodes_[removed_node_index]);
            graph()->mutable_node()->SwapElements(
                last_node_index,
                removed_node_index);
            node_index_by_name_.find(nodes_[removed_node_index].GetName())
                ->second = removed_node_index;
        }
        nodes_.pop_back();
    }
    if (!sorted_node_indices_to_remove.empty())
    {
        ...
        graph()->mutable_node()->DeleteSubrange(
    ...
    absl::Span<const TopologicalDependency> extra_dependencies)
{
    ...
    auto process_fanouts =
        [this, &extra_dependencies_by_parent, &push_onto_stack](
            const int curr_index,
            std::vector<RecursionStackEntry>* recursion_stack,
            std::vector<TraversalState>* traversal_state,
            std::vector<Edge>* edges_in_cycle)
    ...
    auto reversed_postorder_dfs =
        [&process_fanouts](
            const MutableNodeView& root_node_view,
            ...
            std::vector<Edge>* edges_in_cycle)
    {
        ...
        while (!recursion_stack.empty())
        {
            ...
            else if (curr_entry.recursion_state == EXIT)
            {
                ...
                // index would be, so there will not be a need for inversion
                // later on. The value set is in decending order so the reversed
    ...
    if (!ignore_cycles && !edges_in_cycle.empty())
    {
        ...
        {
            edges_formatted.push_back(absl::StrCat(
                "'",
                graph_->node(edge.from).name(),
                "' -> '",
    ...
    for (MutableNodeView& node_view : nodes_)
    {
        ...
        for (MutableFanoutView& controlling_fanin :
             node_view.controlling_fanins_)
        {
            controlling_fanin.node_index_ =
                order[controlling_fanin.node_index_];
        }
        ...
        for (MutableFaninView& controlled_fanout :
             node_view.controlled_fanouts_)
        {
            controlled_fanout.node_index_ =
                order[controlled_fanout.node_index_];
        }
    }
...
inline Status MutableGraphView::ValidateInternal(
    ...
    std::vector<int>* empty_diff_node_indices)
{
    // Get node names and partition updated_nodes_ by if they are renamed or
    // not, skipping empty MutableNodeViewDiff.
    TF_RETURN_IF_ERROR(GetNodeNamesAndPartitionUpdatedNodes(
        node_names,
        renamed_nodes,
        inplace_nodes,
        empty_diff_node_indices));
...
{
    // Node name -> node index mapping. If a node index is -1, the associated
    // node with key node name exists. Otherwise the node index is the node's
    // index in the graph.
    absl::flat_hash_map<absl::string_view, int> node_names;
    // Indices of MutableNodeViewDiff in Mutation::updated_nodes_ where nodes
    // are renamed (and possibly have other fields mutated).
    std::vector<RenamedOrOverwrittenNode> renamed_nodes;
    // Indices of MutableNodeViewDiff in Mutation::updated_nodes_ where nodes
    // are not renamed but have fields mutated.
    std::vector<int> inplace_nodes;
    // Indices of nodes in graph where MutableNodeViewDiff are empty.
    // `update_index_` of nodes associated to empty MutableNodeViewDiff should
    // be cleared after validation success.
    std::vector<int> empty_diff_node_indices;
    ...
    {
        nodes_[empty_diff_node_index].update_index_ = kMissingIndex;
    }
