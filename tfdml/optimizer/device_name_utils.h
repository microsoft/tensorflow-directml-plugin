...
class DeviceNameUtils
{
  public:
    ...
    struct ParsedName
    {
        ...
        {
            return (has_job ? (other.has_job && job == other.job)
                            : !other.has_job) &&
                   (has_replica
                        ? (other.has_replica && replica == other.replica)
                        : !other.has_replica) &&
    ...
    static bool ParseFullOrLocalName(
    ...
    static Status CanonicalizeDeviceName(
        absl::string_view fullname,
        absl::string_view basename,
        std::string* canonical_name);

    // Returns true if "name" specifies any non-trivial constraint on the
    // device.
    static bool HasSomeDetails(const ParsedName& name)
    {
        return name.has_job || name.has_replica || name.has_task ||
    ...
    static void EnsureSpecification(
    ...
    static bool IsCompleteSpecification(
    ...
    static Status MergeOverrideDevNames(
        ParsedName* target,
        const ParsedName& other);
    ...
    // same address space.
    static bool IsSameAddressSpace(
        absl::string_view src,
        absl::string_view dst);
    static bool IsSameAddressSpace(
        const ParsedName& src,
        const ParsedName& dst);

    // Returns true iff devices identified by 'a' and 'b' are in different
    // address space.
    static bool IsDifferentAddressSpace(
        const ParsedName& a,
        const ParsedName& b);
    ...
    // 'pn'. The returned string names are often useful to look up devices from
    // a mapping.
