...
class LstmInitHelper : public InitializationHelper
{
  public:
        ...
        : attr_(std::move(attr))
    {
        ...
        const Tensor& cs_prev_tensor = ctx->input(1);
        const Tensor& h_prev_tensor = ctx->input(2);
        const Tensor& w_tensor = ctx->input(3);
        const Tensor& wci_tensor = ctx->input(4);
        const Tensor& wcf_tensor = ctx->input(5);
        const Tensor& wco_tensor = ctx->input(6);
        const Tensor& b_tensor = ctx->input(7);
        ...
        OP_REQUIRES(
            ctx,
            cs_prev_tensor.dim_size(0) == batch_size_,
        ...
        OP_REQUIRES(
            ctx,
            cs_prev_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
        ...
        OP_REQUIRES(
            ctx,
            h_prev_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
        ...
        OP_REQUIRES(
            ctx,
            h_prev_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
        ...
        OP_REQUIRES(
            ctx,
            w_tensor.dim_size(0) == input_size_ + cell_size_,
            errors::InvalidArgument(
        ...
        OP_REQUIRES(
            ctx,
            w_tensor.dim_size(1) == cell_size_ * 4,
            errors::InvalidArgument(
                "w.dim_size(1) != cell_size * 4: ",
                w_tensor.dim_size(1),
                " vs. ",
                cell_size_ * 4));
        ...
        OP_REQUIRES(
            ctx,
            b_tensor.dim_size(0) == cell_size_ * 4,
            errors::InvalidArgument(
                "b.dim_size(0) != cell_size * 4: ",
                b_tensor.dim_size(0),
                " vs. ",
                cell_size_ * 4));
    }
    ...
    int64_t GetInputSize() const { return input_size_; }
    int64_t GetCellSize() const { return cell_size_; }
  ...
  private:
    ...
    int64_t cell_size_ = 0;
};

class LstmShapeHelper : public ShapeHelper
{
  public:
    using InitHelper = LstmInitHelper;
  ...
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        if (use_peephole)
        {
            ...
            wci_info.desc = DmlTensorDesc::Create(
            ...
            wcf_info.desc = DmlTensorDesc::Create(
                ctx->GetInputDataType(5),
                outshape,
                inshape);
            ...
            wco_info.desc = DmlTensorDesc::Create(
                ctx->GetInputDataType(6),
                outshape,
                inshape);
        ...
        auto forget_bias_tensor = dml::ScalarTensor(
            scope,
            TfTensorTypeTraits<T>::FromFloat(forget_bias),
...
class LstmGradInitHelper : public InitializationHelper
{
  public:
        ...
        : attr_(std::move(attr))
    {
        ...
        const Tensor& wco_tensor = ctx->input(6);
        const Tensor& b_tensor = ctx->input(7);
        const Tensor& i_tensor = ctx->input(8);
        const Tensor& cs_tensor = ctx->input(9);
        const Tensor& f_tensor = ctx->input(10);
        const Tensor& o_tensor = ctx->input(11);
        const Tensor& ci_tensor = ctx->input(12);
        const Tensor& co_tensor = ctx->input(13);
        const Tensor& cs_grad_tensor = ctx->input(14);
        const Tensor& h_grad_tensor = ctx->input(15);

        batch_size_ = x_tensor.dim_size(0);
        input_size_ = x_tensor.dim_size(1);
        cell_size_ = cs_prev_tensor.dim_size(1);
        wci_shape_ = wci_tensor.shape();
        while (wci_shape_.dims() < 4)
        {
            wci_shape_.InsertDim(0, 1);
        }
        wcf_shape_ = wcf_tensor.shape();
        while (wcf_shape_.dims() < 4)
        {
            wcf_shape_.InsertDim(0, 1);
        }
        wco_shape_ = wco_tensor.shape();
        while (wco_shape_.dims() < 4)
        {
            wco_shape_.InsertDim(0, 1);
        }
        ...
        OP_REQUIRES(
            ctx,
            i_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
                "i.dim_size(0) != batch_size: ",
                i_tensor.dim_size(0),
                " vs. ",
                batch_size_));
        OP_REQUIRES(
            ctx,
            i_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
                "i.dim_size(1) != cell_size: ",
                i_tensor.dim_size(1),
                " vs. ",
                cell_size_));
        ...
        OP_REQUIRES(
            ctx,
            cs_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
                "cs.dim_size(0) != batch_size: ",
                cs_tensor.dim_size(0),
                " vs. ",
                batch_size_));
        OP_REQUIRES(
            ctx,
            cs_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
                "cs.dim_size(1) != cell_size: ",
        ...
        OP_REQUIRES(
            ctx,
            f_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
                "f.dim_size(0) != batch_size: ",
                f_tensor.dim_size(0),
                " vs. ",
                batch_size_));
        OP_REQUIRES(
            ctx,
            f_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
                "i.dim_size(1) != cell_size: ",
                f_tensor.dim_size(1),
                " vs. ",
        ...
        OP_REQUIRES(
            ctx,
            o_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
                "o.dim_size(0) != batch_size: ",
                o_tensor.dim_size(0),
                " vs. ",
                batch_size_));
        OP_REQUIRES(
            ctx,
            o_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
                "o.dim_size(1) != cell_size: ",
                o_tensor.dim_size(1),
                " vs. ",
                cell_size_));
        ...
        OP_REQUIRES(
            ctx,
            ci_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
                "ci.dim_size(0) != batch_size: ",
                ci_tensor.dim_size(0),
                " vs. ",
                batch_size_));
        OP_REQUIRES(
            ctx,
            ci_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
                "ci.dim_size(1) != cell_size: ",
        ...
        OP_REQUIRES(
            ctx,
            co_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
                "co.dim_size(0) != batch_size: ",
                co_tensor.dim_size(0),
                " vs. ",
                batch_size_));
        OP_REQUIRES(
            ctx,
            co_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
                "co.dim_size(1) != cell_size: ",
        ...
        OP_REQUIRES(
            ctx,
            cs_grad_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
        ...
        OP_REQUIRES(
            ctx,
            cs_grad_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
        ...
        OP_REQUIRES(
            ctx,
            h_grad_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
        ...
        OP_REQUIRES(
            ctx,
            h_grad_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
        ...
        absl::Span<const TensorShape> output_shapes) const override
    {
        ...
        // If peephole is not used, last 3 inputs will be empty, so we don't
...
{
  public:
    using InitHelper = LstmGradInitHelper;
    std::vector<TensorShape> GetOutputShapes(
        OpKernelContext* ctx,
        const InitializationHelper* initialization_helper) const override
    {
...
class DmlLstmCellBlockGradOp : public DmlKernel
{
  public:
    using InitHelper = LstmGradInitHelper;
    explicit DmlLstmCellBlockGradOp(
        DmlKernelConstruction* ctx,
        const InitHelper* init_helper)
    {
        ...
        dml::TensorDesc::Dimensions wci_dims(
            wci_shape.begin(),
            wci_shape.end());
        dml::TensorDesc::Dimensions wcf_dims(
            wcf_shape.begin(),
            wcf_shape.end());
        dml::TensorDesc::Dimensions wco_dims(
            wco_shape.begin(),
            wco_shape.end());
        ...
        if (use_peephole)
        {
            ...
            wco_grad =
                dml::Reduce(do_tensor * cs, DML_REDUCE_FUNCTION_SUM, {0});
        }
        else
        {
            wci_grad =
                dml::ZeroTensor(scope, DML_TENSOR_DATA_TYPE_FLOAT32, wci_dims);
            wcf_grad =
                dml::ZeroTensor(scope, DML_TENSOR_DATA_TYPE_FLOAT32, wcf_dims);
            wco_grad =
                dml::ZeroTensor(scope, DML_TENSOR_DATA_TYPE_FLOAT32, wco_dims);
        }

        std::vector<dml::Expression> outputs =
            {cs_prev_grad, dgates, wci_grad, wcf_grad, wco_grad};
        Microsoft::WRL::ComPtr<IDMLCompiledOperator> compiled_op =
            scope.Compile(DML_EXECUTION_FLAG_NONE, outputs);
...
class BlockLstmInitHelper : public InitializationHelper
{
  public:
        ...
        : attr_(std::move(attr))
    {
        ...
        const Tensor& seq_len_max_tensor = ctx->input(0);
        const Tensor& x = ctx->input(1);
        const Tensor& cs_prev_tensor = ctx->input(2);
        const Tensor& h_prev_tensor = ctx->input(3);
        const Tensor& w_tensor = ctx->input(4);
        const Tensor& wci_tensor = ctx->input(5);
        const Tensor& wcf_tensor = ctx->input(6);
        const Tensor& wco_tensor = ctx->input(7);
        const Tensor& b_tensor = ctx->input(8);
        ...
        if (batch_size_ * input_size_ % 2 == 1)
        {
            TF_Log(
                TF_WARNING,
                "BlockLSTMOp is inefficient when both batch_size and "
        ...
        OP_REQUIRES(
            ctx,
            x.dims() == 3,
            errors::InvalidArgument("x must be 3D"));

        OP_REQUIRES(
            ctx,
            cs_prev_tensor.dims() == 2,
        ...
        OP_REQUIRES(
            ctx,
            wci_tensor.dim_size(0) == cell_size_,
            errors::InvalidArgument(
                "wci.dim_size(0) != cell_size: ",
                wci_tensor.dim_size(0),
                " vs. ",
                cell_size_));
        ...
        OP_REQUIRES(
            ctx,
            wcf_tensor.dim_size(0) == cell_size_,
            errors::InvalidArgument(
                "wcf.dim_size(0) != cell_size: ",
                wcf_tensor.dim_size(0),
                " vs. ",
                cell_size_));
        ...
        OP_REQUIRES(
            ctx,
            wco_tensor.dim_size(0) == cell_size_,
            errors::InvalidArgument(
                "wco.dim_size(0) != cell_size: ",
                wco_tensor.dim_size(0),
                " vs. ",
                cell_size_));
    ...
    int64_t GetInputSize() const { return input_size_; }
    int64_t GetCellSize() const { return cell_size_; }
    int64_t GetTimeLength() const { return timelen_; }
    float GetForgetBias() const { return attr_->forget_bias; }
    float GetCellClip() const { return attr_->cell_clip; }
    bool GetUsePeepHole() const { return attr_->use_peephole; }
...
{
  public:
    using InitHelper = BlockLstmInitHelper;
    std::vector<TensorShape> GetOutputShapes(
        OpKernelContext* ctx,
        const InitializationHelper* initialization_helper) const override
    {
...
class DmlBlockLstmOp : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        const int32_t seq_len_max_int =
            static_cast<int32_t>(seq_len_max.base<int64_t>()[0]);
        ...
        dml::TensorDesc::Dimensions output_dims{
            1,
            timelen,
            batch_size,
            cell_size};
        ...
        for (uint32_t t = 0; t < seq_len_max_int; ++t)
        {
            ...
            auto x_tensor =
                dml::Slice(x, tensor_offset, x_extent, slice_stride);
...
class BlockLstmGradInitHelper : public InitializationHelper
{
  public:
        ...
        : attr_(std::move(attr))
    {
        ...
        const Tensor& wco_tensor = ctx->input(7);
        const Tensor& b_tensor = ctx->input(8);
        const Tensor& i_out = ctx->input(9);
        const Tensor& cs_out = ctx->input(10);
        const Tensor& f_out = ctx->input(11);
        const Tensor& o_out = ctx->input(12);
        const Tensor& ci_out = ctx->input(13);
        const Tensor& co_out = ctx->input(14);
        const Tensor& h_out = ctx->input(15);
        const Tensor& cs_grad = ctx->input(16);
        const Tensor& h_grad = ctx->input(17);
        ...
        OP_REQUIRES(
            ctx,
            input_size_ + cell_size_ == w_tensor.dim_size(0),
            errors::InvalidArgument(
                "w matrix rows don't match: ",
                input_size_ + cell_size_,
                " vs. ",
                w_tensor.dim_size(0)));
        ...
        OP_REQUIRES(
            ctx,
            cell_size_ == b_tensor.dim_size(0) / 4,
            errors::InvalidArgument(
...
{
  public:
    using InitHelper = BlockLstmGradInitHelper;
    std::vector<TensorShape> GetOutputShapes(
        OpKernelContext* ctx,
        const InitializationHelper* initialization_helper) const override
    {
...
class DmlBlockLstmGradOp : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        dml::TensorDesc::Dimensions w_dims{
            1,
            1,
        ...
        for (int32_t t = seq_len_max_int - 1; t >= 0; --t)
        {
            ...
            auto i_tensor =
                dml::Slice(i, tensor_offset, output_extent, slice_stride);

            dml::Expression cs_tensor;
            if (use_peephole)
            {
                cs_tensor =
                    dml::Slice(cs, tensor_offset, output_extent, slice_stride);
            }

            auto f_tensor =
                dml::Slice(f, tensor_offset, output_extent, slice_stride);
            auto o_tensor =
                dml::Slice(o, tensor_offset, output_extent, slice_stride);
            auto ci_tensor =
            ...
            // do[t] = sigm'(o[t]) .* dh[t] .* co[t]
            auto do_tensor =
                o_tensor * (1 - o_tensor) * h_grad_tensor * co_tensor;

            // dcs[t] += tanh'(cs[t]) .* dh[t] .* o[t] + dcs[t + 1] .* f[t + 1]
            auto dcs =
                (1 - dml::Pow(co_tensor, 2.0f)) * h_grad_tensor * o_tensor +
                cs_grad_tensor;
...
void RegisterLSTMBlockCell()
{
    using Op = ops::LSTMBlockCell;
    lstm_block<Op, float>::
        template WithTypeConstraint<Op::Attribute::T, TF_FLOAT>::Register();
    lstm_block<Op, Eigen::half>::
        template WithTypeConstraint<Op::Attribute::T, TF_HALF>::Register();
}

void RegisterLSTMBlockCellGrad()
{
    using K = KernelDefinition<
        ops::LSTMBlockCellGrad,
        DmlKernelWrapper<DmlLstmCellBlockGradOp, LstmGradShapeHelper>>;

    RegisterWithTypes<
        K,
        ops::LSTMBlockCellGrad::Attribute::T,
        TF_FLOAT,
        TF_HALF>();
}

template <typename Op, typename DataType, GateLayout gl>
using block_lstm = typename KernelDefinition<
    Op,
    DmlKernelWrapper<DmlBlockLstmOp<DataType, gl>, BlockLstmShapeHelper>>::
    template WithHostMemoryArguments<Op::Argument::seq_len_max>;

void RegisterBlockLSTM()
{
    using Op = ops::BlockLSTM;
    block_lstm<Op, float, ICFO>::
        template WithTypeConstraint<Op::Attribute::T, TF_FLOAT>::Register();
    block_lstm<Op, Eigen::half, ICFO>::
        template WithTypeConstraint<Op::Attribute::T, TF_HALF>::Register();
}

void RegisterBlockLSTMV2()
{
    using Op = ops::BlockLSTMV2;
    block_lstm<Op, float, IFCO>::
        template WithTypeConstraint<Op::Attribute::T, TF_FLOAT>::Register();
    block_lstm<Op, Eigen::half, IFCO>::
        template WithTypeConstraint<Op::Attribute::T, TF_HALF>::Register();
}

template <typename Op, typename DataType, GateLayout gl>
using block_lstm_grad = typename KernelDefinition<
    Op,
    DmlKernelWrapper<
        DmlBlockLstmGradOp<DataType, gl>,
        BlockLstmGradShapeHelper>>::
    template WithHostMemoryArguments<Op::Argument::seq_len_max>;

void RegisterBlockLSTMGrad()
{
    using Op = ops::BlockLSTMGrad;
    block_lstm_grad<Op, float, ICFO>::
        template WithTypeConstraint<Op::Attribute::T, TF_FLOAT>::Register();
    block_lstm_grad<Op, Eigen::half, ICFO>::
        template WithTypeConstraint<Op::Attribute::T, TF_HALF>::Register();
}

void RegisterBlockLSTMGradV2()
{
    using Op = ops::BlockLSTMGradV2;
    block_lstm_grad<Op, float, IFCO>::
        template WithTypeConstraint<Op::Attribute::T, TF_FLOAT>::Register();
    block_lstm_grad<Op, Eigen::half, IFCO>::
        template WithTypeConstraint<Op::Attribute::T, TF_HALF>::Register();
}

void RegisterKernels_LSTM()
{
    RegisterLSTMBlockCell();
    RegisterLSTMBlockCellGrad();
    RegisterBlockLSTM();
    RegisterBlockLSTMV2();
    RegisterBlockLSTMGrad();
    RegisterBlockLSTMGradV2();
}
