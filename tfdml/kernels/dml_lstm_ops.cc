...
template <typename Op, TF_DataType DataType> 
using K = KernelDefinition<Op, DmlKernelWrapper<DmlLstmBlockCellOp<DataType, ICFO>, LstmShapeHelper>>
            ::template WithTypeConstraint<Op::Attribute::T, DataType>;

void RegisterLSTMBlockCell()
{
    using Op = ops::LSTMBlockCell;
    K<Op, TF_FLOAT>::Register();
    K<Op, TF_HALF>::Register();
}
