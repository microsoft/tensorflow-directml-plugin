...
static SimplifiedReverseSequence SimplifyReverseSequence(
    ...
    const TensorShape& seq_lengths_shape,
    int32_t batch_dim,
    int32_t seq_dim)
{
    ...
    int32_t lower_index = std::min(batch_dim, seq_dim);
    int32_t upper_index = std::max(batch_dim, seq_dim);
    ...
    desc.input_output_sizes = {
        dim_before,
        static_cast<uint32_t>(input_shape.dim_size(lower_index)),
        dim_between,
        static_cast<uint32_t>(input_shape.dim_size(upper_index)),
        dim_after,
    };
    ...
    uint32_t new_batch_dim = batch_dim < seq_dim ? 1 : 3;
...
class ReverseSequenceInitHelper : public InitializationHelper
{
  public:
        ...
        std::shared_ptr<const Attributes> attr)
    {
        ...
        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsVector(seq_lens.shape()),
            errors::InvalidArgument(
                "seq_lens input must be 1-dim, not ",
                seq_lens.dims()));

        OP_REQUIRES(
            ctx,
            attr->batch_dim != attr->seq_dim,
            errors::InvalidArgument("batch_dim == seq_dim == ", attr->seq_dim));

        OP_REQUIRES(
            ctx,
            attr->seq_dim < input.dims(),
            errors::InvalidArgument(
                "seq_dim must be < input.dims()",
                "( ",
                attr->seq_dim,
                " vs. ",
                input.dims(),
        ...
        OP_REQUIRES(
            ctx,
            attr->batch_dim < input.dims(),
            errors::InvalidArgument(
                "batch_dim must be < input.dims()",
                "( ",
                attr->batch_dim,
                " vs. ",
                input.dims(),
        ...
        OP_REQUIRES(
            ctx,
            seq_lens.NumElements() == input.dim_size(attr->batch_dim),
            errors::InvalidArgument(
                "len(seq_lens) != input.dims(",
                attr->batch_dim,
                "), ",
                "(",
...
class DmlReverseSequenceKernel : public DmlKernel
{
  public:
    ...
    explicit DmlReverseSequenceKernel(
        DmlKernelConstruction* ctx,
        const InitHelper* init_helper)
    {
        auto simple_reverse_sequence =
            init_helper->GetSimplifiedReverseSequence();
        ...
        seq_lengths.desc = DmlTensorDesc::Create(
            ...
            simple_reverse_sequence.seq_lengths_sizes,
...
void RegisterKernels_ReverseSequence()
{
    using int32_kernel = KernelDefinition<
        ops::ReverseSequence,
        DmlKernelWrapper<
            DmlReverseSequenceKernel,
            GetOutputShapeAsInputShapeHelper>>::
        WithTypeConstraint<ops::ReverseSequence::Attribute::Tlen, TF_INT32>;
    ...
    using int64_kernel = KernelDefinition<
        ...
        DmlKernelWrapper<
            DmlReverseSequenceKernel,
            GetOutputShapeAsInputShapeHelper>>::
        WithTypeConstraint<ops::ReverseSequence::Attribute::Tlen, TF_INT64>;

    RegisterWithTypes<
        int32_kernel,
        ops::ReverseSequence::Attribute::T,
        TF_FLOAT,
        TF_HALF,
        TF_BOOL>();
