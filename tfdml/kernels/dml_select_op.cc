...
absl::optional<SimpleTernary> SimplifyTernary(
    const TensorShape& cond_shape,
    const TensorShape& then_shape,
    const TensorShape& else_shape,
    const TensorShape& output_shape,
    uint32_t output_size = 8)
{
    assert(cond_shape.dims() == then_shape.dims());
    assert(cond_shape.dims() == else_shape.dims());
    assert(cond_shape.dims() == output_shape.dims());
    ...
    // coalesced size until a dimension is reached that cannot be coalesced,
    // then write out the coalesced dimension and the non-coalesced dimension.
    for (int read_dim = output_shape.dims() - 1; read_dim >= 0; read_dim--)
    {
        ...
        auto output_size =
            static_cast<uint32_t>(output_shape.dim_size(read_dim));
    ...
    // TODO: TFDML #25934615
    // Consider improving to handle groups of adjacent dims that can be
    // coaleseced because their products are equal or 1.
...
class SelectInitHelper : public BaseSelectInitHelper
{
  public:
        ...
        std::shared_ptr<const Attributes> attr)
    {
        ...
        if (TensorShapeUtils::IsScalar(cond_shape))
        {
            OP_REQUIRES_OK(
                ctx,
                ComputeScalarSelectShapes(cond_shape, then_shape, else_shape));
        }
        else
        {
            bool broadcasting =
            ...
            else
            {
                OP_REQUIRES_OK(
                    ctx,
                    ComputeElementWiseSelectShapes(
                        cond_shape,
                        then_shape,
                        else_shape));
            }
  ...
  private:
    Status ComputeScalarSelectShapes(
        const TensorShape& cond_shape,
        const TensorShape& then_shape,
        const TensorShape& else_shape)
    {
    ...
    Status ComputeBatchSelectShapes(
        const TensorShape& cond_shape,
        const TensorShape& then_shape,
        const TensorShape& else_shape)
    {
        ...
        {
            return errors::InvalidArgument(
                "'cond' must be a vector, but saw shape: ",
        ...
        {
            return errors::InvalidArgument(
                "Number of batches of 'then' must match size of 'cond', but "
                "saw: ",
                then_shape.dim_size(0),
                " vs. ",
    ...
    Status ComputeElementWiseSelectShapes(
        const TensorShape& cond_shape,
        const TensorShape& then_shape,
        const TensorShape& else_shape)
    {
        ...
        {
            return errors::InvalidArgument(
                "'cond', 'then' and 'else' must have the same size when 'cond' "
                "is "
...
class SelectV2InitHelper : public BaseSelectInitHelper
{
  public:
        ...
        std::shared_ptr<const Attributes> attr)
    {
        ...
        OP_REQUIRES(
            ctx,
            broadcasted_output_shape_.dims() <= 8,
            errors::InvalidArgument(
...
class DmlTernaryKernel : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        cond_input.desc = DmlTensorDesc::Create(
        ...
        then_input.desc = DmlTensorDesc::Create(
        ...
        else_input.desc = DmlTensorDesc::Create(
...
void RegisterSelect()
{
    using K = KernelDefinition<
        ops::Select,
        DmlKernelWrapper<
            DmlTernaryKernel<SelectInitHelper>,
            GetSelectOutputShapeHelper>>;

    RegisterWithTypes<
        K,
        ops::Select::Attribute::T,
        TF_FLOAT,
        TF_HALF,
        TF_BOOL,
        TF_INT32,
...
void RegisterSelectV2()
{
    using K = KernelDefinition<
        ops::SelectV2,
        DmlKernelWrapper<
            DmlTernaryKernel<SelectV2InitHelper>,
            GetSelectOutputShapeHelper>>;

    RegisterWithTypes<
        K,
        ops::SelectV2::Attribute::T,
        TF_FLOAT,
        TF_HALF,
        TF_BOOL,
        TF_INT32,
