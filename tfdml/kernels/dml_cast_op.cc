 ...
 public:
  ...
  StatusOr<DmlGpuEvent> Compute(DmlKernelContext* ctx) const {
    if (zero_outputs_) {
      ...
      ctx->GetDmlDeviceContext()->ZeroBuffer(ctx->GetDmlDeviceContext()->GetBufferForTensor(output));
    }
...
template <TF_DataType T, TF_DataType... Ts> void RegisterCast()
{
    using Op = ops::Cast;
    ...
    K::template WithTypeConstraint<Op::Attribute::SrcT, TF_FLOAT>::Register();
    K::template WithTypeConstraint<Op::Attribute::SrcT, TF_HALF>::Register();
    K::template WithTypeConstraint<Op::Attribute::SrcT, TF_BOOL>::Register();
    K::template WithTypeConstraint<Op::Attribute::SrcT, TF_UINT8>::Register();
    K::template WithTypeConstraint<Op::Attribute::SrcT, TF_INT32>::Register();
    K::template WithTypeConstraint<Op::Attribute::SrcT, TF_INT64>::Register();

    if constexpr (sizeof...(Ts) > 0) RegisterCast<Ts...>();
}

void RegisterKernels_Cast()
{
    RegisterCast<TF_FLOAT, TF_HALF, TF_BOOL, TF_UINT8, TF_INT32, TF_INT64>();
}
