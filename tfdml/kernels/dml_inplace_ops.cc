...
class InplaceInitializationHelper : public InitializationHelper
{
  public:
        ...
        std::shared_ptr<const Attributes> attr)
    {
        ...
        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsVector(i.shape()),
        ...
        for (int i = 1; i < x.dims(); ++i)
        {
            OP_REQUIRES(
                ctx,
                x.dim_size(i) == v.dim_size(i),
                errors::InvalidArgument(
                    "x and v shape doesn't match at index ",
                    i,
                    " : ",
                    x.shape().DebugString(),
                    " vs. ",
...
class DmlInplaceKernel : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        indices.desc = DmlTensorDesc::Create(
...
struct InplaceAddFunctor
{
    dml::Expression operator()(
        ...
        const dml::Expression& updates)
    {
        const auto zero = dml::ZeroTensor(
            scope,
...
void RegisterInplaceUpdate()
{
    using K = KernelDefinition<
        ops::InplaceUpdate,
        DmlKernelWrapper<
            DmlInplaceKernel<InplaceUpdateFunctor>,
            GetOutputShapeAsInputShapeHelper>>;

    RegisterWithTypes<
        K,
        ops::InplaceUpdate::Attribute::T,
        TF_FLOAT,
        TF_HALF,
        TF_INT64>();
}

void RegisterInplaceAdd()
{
    using K = KernelDefinition<
        ops::InplaceAdd,
        DmlKernelWrapper<
            DmlInplaceKernel<InplaceAddFunctor>,
            GetOutputShapeAsInputShapeHelper>>;

    RegisterWithTypes<
        K,
        ops::InplaceAdd::Attribute::T,
        TF_FLOAT,
        TF_HALF,
        TF_INT64>();
}

void RegisterInplaceSub()
{
    using K = KernelDefinition<
        ops::InplaceSub,
        DmlKernelWrapper<
            DmlInplaceKernel<InplaceSubFunctor>,
            GetOutputShapeAsInputShapeHelper>>;

    RegisterWithTypes<
        K,
        ops::InplaceSub::Attribute::T,
        TF_FLOAT,
        TF_HALF,
        TF_INT64>();
}
