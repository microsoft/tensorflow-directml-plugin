...
static inline void ParseAttributeVec4(
    ...
    /*out*/ absl::Span<uint32_t> attr)
{
    ...
    OP_REQUIRES(
        ctx,
        (int32_attr)[0] == 1 && (int32_attr)[3] == 1,
        errors::Unimplemented("Only support ", attr_name, " across space."));
    OP_REQUIRES(
        ctx,
        (int32_attr)[1] >= 1 && (int32_attr)[2] >= 1,
...
class ExtractImagePatchesInitHelper : public InitializationHelper
{
  public:
    ...
    explicit ExtractImagePatchesInitHelper(
        OpKernelContext* ctx,
        ...
        : attr_(std::move(attr))
    {
        ...
        const int ksize_rows_eff =
            ksize_rows + (ksize_rows - 1) * (rate_rows - 1);
        const int ksize_cols_eff =
            ksize_cols + (ksize_cols - 1) * (rate_cols - 1);

        int64_t out_rows, out_cols;
        int64_t rows_padding_before, cols_padding_before;
        int64_t rows_padding_after, cols_padding_after;
...
class DmlExtractImagePatchesKernel : public DmlKernel
{
  public:
    ...
    explicit DmlExtractImagePatchesKernel(
...
void RegisterKernels_ExtractImagePatches()
{
    using K = KernelDefinition<
        ...
        DmlKernelWrapper<
            DmlExtractImagePatchesKernel,
            ExtractImagePatchesShapeHelper>>;

    RegisterWithTypes<
        K,
        ops::ExtractImagePatches::Attribute::T,
        TF_FLOAT,
        TF_HALF>();
}
