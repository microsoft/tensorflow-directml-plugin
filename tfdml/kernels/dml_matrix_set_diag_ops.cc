...
class MatrixSetDiagInitHelper : public InitializationHelper
{
  public:
        ...
        std::shared_ptr<const Attributes> attr)
    {
        ...
        // MatrixSetDiag, MatrixSetDiagV2, and MatrixSetDiagV3 both use this
        // OpKernel. MatrixSetDiag only has two inputs, so we have to check the
        // number of inputs before reading additional parameters in
        // MatrixSetDiagV2/MatrixSetDiagV3.
        ...
        if (ctx->num_inputs() > 2)
        {
            const Tensor& diag_index = ctx->input(2);
            OP_REQUIRES(
                ctx,
        ...
        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsVectorOrHigher(diag_shape),
            errors::InvalidArgument(
        ...
        OP_REQUIRES(
            ctx,
            expected_diag_shape == diag_shape,
            errors::InvalidArgument(
                "Either first dimensions of diagonal don't match "
                "input.shape[:-2], "
                "or diagonal.shape[:-1] is not equal to the longests diagonal "
                "in "
                "range [lower_diag_index:upper_diag_index].\nInput shape: ",
                input_shape.DebugString(),
                "\nDiagonal shape: ",
                diag_shape.DebugString(),
                "\nExpected diagonal shape: ",
                expected_diag_shape.DebugString()));
...
class DmlMatrixSetDiagKernel : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        const TensorShape flattened_in_out_shape(
            {1, batch_size, height, width});
