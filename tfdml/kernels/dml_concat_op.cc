...
class ConcatInitHelper : public InitializationHelper
{
  public:
        ...
        std::shared_ptr<const Attributes> attr)
    {
        ...
        {
            CHECK(
                concat_dim_tensor.dtype() == TF_INT32 ||
        ...
        OP_REQUIRES(
            ctx,
            0 <= concat_axis_ && concat_axis_ < input_dims,
        ...
        for (int i = 1; i < values.size(); ++i)
        {
            ...
            OP_REQUIRES(
                ...
                in.dims() == input_dims,
                errors::InvalidArgument(
                    "ConcatOp : Ranks of all input tensors should match: "
                    "shape[0] = ",
                    first_input_shape_.DebugString(),
                    " vs. shape[",
                    i,
                    "] = ",
            ...
            for (int j = 0; j < in.dims(); ++j)
            {
                if (j != concat_axis_)
                {
                    OP_REQUIRES(
                        ctx,
                        in.dim_size(j) == first_input_shape_.dim_size(j),
                        errors::InvalidArgument(
...
class ConcatShapeHelper : public ShapeHelper
{
  public:
        ...
        const InitializationHelper* initialization_helper) const override
    {
        ...
        const TensorShape& first_input_shape =
            init_helper->GetFirstInputShape();
...
class DmlConcatKernel : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        // the kernel input indices (which is what determines the mapping
        // between kernel inputs and DML op inputs)
        for (uint32_t i = 0; i < ctx->GetInputCount(); ++i)
        {
            ...
            int axis_dim_size =
                ctx->GetInputTensorShape(i).dim_size(concat_axis);
        ...
        // If all tensors are empty, this kernel should have already been
        // no-op'd earlier
