...
class GruInitHelper : public InitializationHelper
{
  public:
    ...
    GruInitHelper(OpKernelContext* ctx, std::shared_ptr<const Attributes> attr)
    {
        ...
        const Tensor& h_prev_tensor = ctx->input(1);
        const Tensor& w_ru_tensor = ctx->input(2);
        const Tensor& w_c_tensor = ctx->input(3);
        const Tensor& b_ru_tensor = ctx->input(4);
        const Tensor& b_c_tensor = ctx->input(5);
        ...
        OP_REQUIRES(
            ctx,
            w_ru_tensor.dim_size(0) == input_size_ + cell_size_,
            errors::InvalidArgument(
                "w_ru.dim_size(0) != input_size + cell_size: ",
                w_ru_tensor.dim_size(0),
                " vs. ",
                input_size_ + cell_size_));
        OP_REQUIRES(
            ctx,
            w_ru_tensor.dim_size(1) == cell_size_ * 2,
            errors::InvalidArgument(
                "w_ru.dim_size(1) != cell_size * 2: ",
                w_ru_tensor.dim_size(1),
                " vs. ",
                cell_size_ * 2));
        ...
        OP_REQUIRES(
            ctx,
            w_c_tensor.dim_size(0) == input_size_ + cell_size_,
            errors::InvalidArgument(
                "w_c.dim_size(0) != input_size + cell_size: ",
                w_c_tensor.dim_size(0),
                " vs. ",
                input_size_ + cell_size_));
        OP_REQUIRES(
            ctx,
            w_c_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
                "w_c.dim_size(1) != cell_size: ",
                w_c_tensor.dim_size(1),
                " vs. ",
                cell_size_));
        ...
        OP_REQUIRES(
            ctx,
            b_ru_tensor.dim_size(0) == cell_size_ * 2,
            errors::InvalidArgument(
                "b_ru.dim_size(0) != cell_size * 2: ",
                b_ru_tensor.dim_size(0),
                " vs. ",
                cell_size_ * 2));
        OP_REQUIRES(
            ctx,
        ...
        OP_REQUIRES(
            ctx,
            b_c_tensor.dim_size(0) == cell_size_,
            errors::InvalidArgument(
                "b_c.dim_size(0) != cell_size: ",
                b_c_tensor.dim_size(0),
                " vs. ",
                cell_size_));
        OP_REQUIRES(
            ctx,
...
{
  public:
    using InitHelper = GruInitHelper;
...
class DmlGruBlockCellOp : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        dml::TensorDesc::Dimensions cell_extents =
            {1, 1, batch_size, cell_size};
...
class GruGradInitHelper : public InitializationHelper
{
  public:
        ...
        std::shared_ptr<const Attributes> attr)
    {
        ...
        const Tensor& b_ru_tensor = ctx->input(4);
        const Tensor& b_c_tensor = ctx->input(5);
        const Tensor& r_tensor = ctx->input(6);
        const Tensor& u_tensor = ctx->input(7);
        const Tensor& c_tensor = ctx->input(8);
        const Tensor& d_h_tensor = ctx->input(9);
        ...
        OP_REQUIRES(
            ctx,
            r_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
                "r.dims(0) != batch_size: ",
                r_tensor.dim_size(0),
                " vs. ",
        ...
        OP_REQUIRES(
            ctx,
            r_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
                "r.dims(1) != cell_size: ",
                r_tensor.dim_size(1),
                " vs. ",
        ...
        OP_REQUIRES(
            ctx,
            u_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
                "u.dims(0) != batch_size: ",
                u_tensor.dim_size(0),
                " vs. ",
        ...
        OP_REQUIRES(
            ctx,
            u_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
                "u.dims(1) != cell_size: ",
                u_tensor.dim_size(1),
                " vs. ",
        ...
        OP_REQUIRES(
            ctx,
            c_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
                "c.dims(0) != batch_size: ",
                c_tensor.dim_size(0),
                " vs. ",
        ...
        OP_REQUIRES(
            ctx,
            c_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
                "c.dims(1) != cell_size: ",
                c_tensor.dim_size(1),
                " vs. ",
        ...
        OP_REQUIRES(
            ctx,
            d_h_tensor.dim_size(0) == batch_size_,
            errors::InvalidArgument(
                "d_h.dims(0) != batch_size: ",
                d_h_tensor.dim_size(0),
                " vs. ",
                batch_size_));
        OP_REQUIRES(
            ctx,
            d_h_tensor.dim_size(1) == cell_size_,
            errors::InvalidArgument(
                "d_h.dims(1) != cell_size: ",
                d_h_tensor.dim_size(1),
                " vs. ",
                cell_size_));
    }
...
{
  public:
    using InitHelper = GruGradInitHelper;
    std::vector<TensorShape> GetOutputShapes(
        OpKernelContext* ctx,
        const InitializationHelper* initialization_helper) const override
    {
...
class DmlGruCellBlockGradOp : public DmlKernel
{
  public:
    using InitHelper = GruGradInitHelper;
    explicit DmlGruCellBlockGradOp(
        DmlKernelConstruction* ctx,
        const InitHelper* init_helper)
    {
        ...
        params.kernel_input_indices =
            absl::InlinedVector<absl::optional<uint32_t>, 8>(
                {1, 2, 3, 6, 7, 8, 9});
        auto tensors = GetTensorInfos(ctx, params);
        ...
        auto d_x_comp2_and_h_prevr_gemm = dml::Gemm(
            d_c_bar,
            w_c,
            dml::NullOpt,
        ...
        // [1st_component_of_d_x 1st_component_of_d_h_prev] = [d_r_bar d_u_bar]
        // X w_ru^T
        auto d_x_comp1_and_h_prev_comp1_gemm = dml::Gemm(
            d_r_bar_u_bar,
            w_ru,
            dml::NullOpt,
            DML_MATRIX_TRANSFORM_NONE,
        ...
        auto comp1_plus_comp2 =
            d_x_comp1_and_h_prev_comp1 + d_x_comp2_and_h_prevr;

        // d_x = d_x_comp1 + d_x_comp2
        auto d_x =
            dml::Slice(comp1_plus_comp2, x_offset, x_extent, slice_stride);
        // d_h_prev = d_h_comp1 + d_hr*r + d_h*u
        auto d_h_prev = dml::Slice(
...
void RegisterGRUBlockCell()
{
    using K = KernelDefinition<
        ops::GRUBlockCell,
        DmlKernelWrapper<DmlGruBlockCellOp, GruShapeHelper>>;

    RegisterWithTypes<K, ops::GRUBlockCell::Attribute::T, TF_FLOAT>();
}

void RegisterGRUBlockCellGrad()
{
    using K = KernelDefinition<
        ops::GRUBlockCellGrad,
        DmlKernelWrapper<DmlGruCellBlockGradOp, GruGradShapeHelper>>;

    RegisterWithTypes<K, ops::GRUBlockCellGrad::Attribute::T, TF_FLOAT>();
}

void RegisterKernels_GRU()
{
    RegisterGRUBlockCell();
    RegisterGRUBlockCellGrad();
}
