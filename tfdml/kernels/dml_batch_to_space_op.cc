...
class BaseBatchToSpaceInitHelper : public InitializationHelper {
 public:
  ...
  absl::Span<const int64_t> GetInternalCrops() const { return internal_crops_; }
 ...
 public:
  ...
  DmlBatchToSpaceKernel(DmlKernelConstruction* ctx,
    ...
    if (init_helper->IsSliceRequired()) {
      ...
      absl::Span<const int64_t> internal_crops = init_helper->GetInternalCrops();
...
static void RegisterBatchToSpaceND()
{
    using K = KernelDefinition<
        ops::BatchToSpaceND,
        DmlKernelWrapper<DmlBatchToSpaceKernel<BatchToSpaceNdInitHelper>, BatchToSpaceShapeHelper>>
        ::template WithHostMemoryArguments<ops::BatchToSpaceND::Argument::block_shape>
        ::template WithHostMemoryArguments<ops::BatchToSpaceND::Argument::crops>;

    RegisterWithTypes<K, ops::BatchToSpaceND::Attribute::T, TF_FLOAT, TF_HALF>();
}
...
static void RegisterBatchToSpace()
{
    using K = KernelDefinition<
        ops::BatchToSpace,
        DmlKernelWrapper<DmlBatchToSpaceKernel<BatchToSpaceInitHelper>, BatchToSpaceShapeHelper>>
