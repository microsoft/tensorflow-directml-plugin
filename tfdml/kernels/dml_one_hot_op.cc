  ...
  public:
        ...
        : attr_(attr)
    {
        ...
        OP_REQUIRES(
            ctx,
            attr_->axis == -1 ||
                (attr_->axis >= 0 && attr_->axis < output_dims),
            errors::InvalidArgument(
                "Expected axis to be -1 or between [0, ",
                output_dims,
                ").  But received: ",
                attr_->axis));
        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsScalar(depth_tensor.shape()),
            errors::InvalidArgument(
                "depth must be a scalar, but got: ",
                depth_tensor.shape().DebugString()));
        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsScalar(on_value.shape()),
            errors::InvalidArgument(
                "on_value must be a scalar, but got: ",
                on_value.shape().DebugString()));
        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsScalar(off_value.shape()),
            errors::InvalidArgument(
                "off_value must be a scalar, but got: ",
                off_value.shape().DebugString()));

        // The one-hot dimension.
        depth_ = depth_tensor.base<int32_t>()[0];
        OP_REQUIRES(
            ctx,
            depth_ >= 0,
            errors::InvalidArgument(
        ...
        OP_REQUIRES(
            ctx,
            MultiplyWithoutOverflow(indices_shape.num_elements(), depth_) >= 0,
            errors::InvalidArgument(
                "OneHot result would have shape ",
...
class DmlOneHotKernel : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        // indices tensor. The +1 comes from inserting one-hot values along a
        // new dimension, with size 'Depth', at a specified axis. For example,
        // consider an indices tensor with shape [2,2,4,3,2]. If axis = 2, then
        // the output shape will be [2,2,Depth,4,3,2]. DML can only handle 4D
        // tensors, but it can support the higher-order shapes by collapsing
        // indices to the left and right of the inserted one-hot dimension. In
        // the above example, the output shape can be collapsed into
        // [4,Depth,24]: 2*2 elements left of axis and 4*3*2 elements right of
        // axis. More generally, where L = elements left of axis and R =
        // elements right of axis:
        // - indices shape = [1,L,1,R]
        // - output shape  = [1,L,Depth,R]
        // Note that L and R will be a minimum of 1, since a dimension cannot
        // have a size of 0. These shapes effectively fix the axis in DML such
        // that it is always 2.
        ...
        const uint32_t depth = static_cast<uint32_t>(
            ctx->GetConstantInputTensor(1).base<int32_t>()[0]);
        ...
        const TF_DataType indices_type = ctx->GetInputDataType(0);
        ...
        // Although DML's OneHot supports int32, the way it handles negative
        // indices is different from TensorFlow. In negative indices in DML are
        // relative to the end, but in TensorFlow they are always out of bounds.
        // By forcing the indices to be unsigned we make sure that they are
        // always out of bounds if their signed representation is negative.
...
{
    using uint8_kernel = KernelDefinition<
        ops::OneHot,
        DmlKernelWrapper<DmlOneHotKernel, OneHotShapeHelper>>::
        WithHostMemoryArguments<ops::OneHot::Argument::depth>::
            WithTypeConstraint<ops::OneHot::Attribute::TI, TF_UINT8>;

    using int32_kernel = KernelDefinition<
        ops::OneHot,
        DmlKernelWrapper<DmlOneHotKernel, OneHotShapeHelper>>::
        WithHostMemoryArguments<ops::OneHot::Argument::depth>::
            WithTypeConstraint<ops::OneHot::Attribute::TI, TF_INT32>;

    using int64_kernel = KernelDefinition<
        ops::OneHot,
        DmlKernelWrapper<DmlOneHotKernel, OneHotShapeHelper>>::
        WithHostMemoryArguments<ops::OneHot::Argument::depth>::
            WithTypeConstraint<ops::OneHot::Attribute::TI, TF_INT64>;

    RegisterWithTypes<
        uint8_kernel,
        ops::OneHot::Attribute::T,
        TF_FLOAT,
        TF_HALF,
        TF_BOOL,
        TF_INT32,
