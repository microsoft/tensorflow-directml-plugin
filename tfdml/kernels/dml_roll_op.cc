...
class RollInitHelper : public InitializationHelper {
 public:
  ...
  RollInitHelper(OpKernelContext* ctx, std::shared_ptr<const Attributes> attr) {
    ...
    auto shift_flat = shift.base<Tshift>();
    auto axis_flat = axis.base<Taxis>();
...
template <typename Op, TF_DataType DataType, typename Tshift, typename Taxis>
using K = typename KernelDefinition<Op, DmlKernelWrapper<DmlRollKernel<Tshift, Taxis>, GetOutputShapeAsInputShapeHelper>> 
    ::template WithTypeConstraint<Op::Attribute::T, DataType>
    ::template WithTypeConstraint<Op::Attribute::Tshift, DataTypeToEnum<Tshift>>
    ::template WithTypeConstraint<Op::Attribute::Taxis, DataTypeToEnum<Taxis>>                    
...
{
    using Op = ops::Roll;
    K<Op, T, int32_t, int32_t>::Register();
    K<Op, T, int64_t, int32_t>::Register();
    K<Op, T, int32_t, int64_t>::Register();
    K<Op, T, int64_t, int64_t>::Register();
