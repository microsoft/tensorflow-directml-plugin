...
class RollInitHelper : public InitializationHelper
{
  public:
    ...
    RollInitHelper(OpKernelContext* ctx, std::shared_ptr<const Attributes> attr)
    {
        ...
        auto shift_flat = reinterpret_cast<const Tshift*>(shift.raw_data());
        auto axis_flat = reinterpret_cast<const Taxis*>(axis.raw_data());

        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsVectorOrHigher(input.shape()),
        ...
        OP_REQUIRES(
            ctx,
            shift.shape() == axis.shape(),
            errors::InvalidArgument("shift and axis must have the same size"));
        const int num_shifts = static_cast<int>(shift.NumElements());
        ...
        for (int i = 0; i < num_shifts; i++)
        {
            ...
            const int ds =
                std::max<int>(static_cast<int>(input.dim_size(axis)), 1);
            const int sum =
                shift_mod_sum_[axis] + static_cast<int>(shift_flat[i]);
            // modulo that works with negatives: ((x % y) + y) % y
            shift_mod_sum_[axis] = (sum % ds + ds) % ds;
        }
    }

    absl::Span<const int32_t> GetShiftModSum() const { return shift_mod_sum_; }

  private:
    absl::InlinedVector<int32_t, 4> shift_mod_sum_;
};
...
class DmlRollKernel : public DmlKernel
{
  public:
    ...
    DmlRollKernel(DmlKernelConstruction* ctx, const InitHelper* init_helper)
    {
        ...
        absl::Span<const int32_t> shift_mod_sum = init_helper->GetShiftModSum();
        bool is_identity = std::all_of(
            shift_mod_sum.begin(),
            shift_mod_sum.end(),
            [](int32_t axis) { return axis == 0; });
        ...
        for (int axis = 0; axis < shift_mod_sum.size(); ++axis)
        {
            ...
            // Coalesce the non-axis dimensions to the left and to the right
            // together to allow N dimensional support
            ...
            dml::TensorDesc::Dimensions new_sizes =
                {1, before_dim_size, axis_dim_size, after_dim_size};

            // Because of the int64 stride hack, the first int64 reinterpret
            // needs to take the strides into account since we didn't override
            // the result with a non-strided gather yet TFDML #24881131
...
template <typename Op, typename Tshift, typename Taxis>
using K = typename KernelDefinition<
    Op,
    DmlKernelWrapper<
        DmlRollKernel<Tshift, Taxis>,
        GetOutputShapeAsInputShapeHelper>>::
    template WithTypeConstraint<
        Op::Attribute::Tshift,
        DataTypeToEnum<Tshift>()>::
        template WithTypeConstraint<
            Op::Attribute::Taxis,
            DataTypeToEnum<Taxis>()>::
...
{
    RegisterRoll<TF_FLOAT, TF_HALF, TF_INT32, TF_INT64>();
}
