        ...
        std::shared_ptr<const Attributes> attr)
    {
        ...
        OP_REQUIRES(
            ...
            errors::InvalidArgument(
                "k must be 0-D, got shape ",
                k_in.shape().DebugString()));
        OP_REQUIRES(
            ctx,
            predictions_in.dims() == 2,
        ...
        OP_REQUIRES(
            ctx,
            predictions_in.dim_size(0) == targets_in.dim_size(0),
            errors::InvalidArgument(
...
class DmlInTopKKernel : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        targets_info.desc = DmlTensorDesc::Create(
        ...
        {
            classes =
                dml::Sequence<int32_t>(scope, 0, 1, {1, 1, 1, num_classes});
            num_classes_tensor = dml::ScalarTensor<int32_t>(
                scope,
                num_classes,
                targets.GetOutputDesc().sizes);
            zero_tensor = dml::ScalarTensor<int32_t>(
                scope,
                0,
                targets.GetOutputDesc().sizes);
        }
        else
        {
            ...
            classes =
                dml::Sequence<int64_t>(scope, 0, 1, {1, 1, 1, num_classes});
            num_classes_tensor = dml::ScalarTensor<int64_t>(
                scope,
                num_classes,
                targets.GetOutputDesc().sizes);
            zero_tensor = dml::ScalarTensor<int64_t>(
                scope,
                0,
                targets.GetOutputDesc().sizes);
        }
        ...
        auto sparse_classes = dml::Reinterpret(
            classes,
            predictions.GetOutputDesc().sizes,
            dml::TensorStrides({0, 0, 0, 1}));

        auto zero_float_tensor = dml::ScalarTensor<float>(
            scope,
            0.0,
            predictions.GetOutputDesc().sizes);

        auto sparse_targets = dml::Reinterpret(
            targets,
            predictions.GetOutputDesc().sizes,
        ...
        auto sparse_predictions = dml::If(
            sparse_targets == sparse_classes,
            predictions,
            zero_float_tensor);
        ...
        lowest_topk_value_per_batch = dml::Reinterpret(
        ...
        auto result =
            targets >= zero_tensor && targets < num_classes_tensor &&
...
static void RegisterInTopKV2()
{
    // TODO: Remove k pinning to host once we have an InTopKV2 DML kernel and K
    // is moved into device memory
    using K = KernelDefinition<
        ops::InTopKV2,
        DmlKernelWrapper<
            DmlInTopKKernel,
            GetOutputShapeFromInputShapeHelper<1>>>::
        WithHostMemoryArguments<ops::InTopKV2::Argument::k>;

    RegisterWithTypes<K, ops::InTopKV2::Attribute::T, TF_INT32, TF_INT64>();
}

void RegisterKernels_InTopK() { RegisterInTopKV2(); }
