  ...
  public:
        ...
        : attr_(std::move(attr))
    {
        ...
        OP_REQUIRES(
            ...
            crop_size.dims() == 1,
            errors::InvalidArgument(
                "crop_size must be 1-D",
                crop_size.shape().DebugString()));

        OP_REQUIRES(
            ctx,
            crop_size.dim_size(0) == 2,
            errors::InvalidArgument(
                "crop_size must have two elements",
                crop_size.shape().DebugString()));

        // Copy and validate crop sizes.
        auto crop_size_vec = crop_size.base<int32_t>();
        ...
        output_shape_ =
            TensorShape({num_boxes, crop_height, crop_width, depth});
    }
...
class DmlCropAndResizeKernel : public DmlKernel
{
  public:
    ...
    explicit DmlCropAndResizeKernel(
...
void RegisterKernels_CropAndResize()
{
    using K = KernelDefinition<
        ...
        WithHostMemoryArguments<ops::CropAndResize::Argument::crop_size>;

    RegisterWithTypes<K, ops::CropAndResize::Attribute::T, TF_FLOAT, TF_HALF>();
}
