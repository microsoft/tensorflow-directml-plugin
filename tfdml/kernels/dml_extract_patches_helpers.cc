    ...
    absl::Span<const uint32_t> output_sizes)
{
    ...
    for (int i = spatial_dim_count; i > 0; --i)
    {
        ...
        {
            // Indices of the patches in the input (first patch is at index 0,
            // and last patch is at index out_size - 1)
            auto patch_indices = dml::Sequence<uint32_t>(
                scope,
                0,
                patch_step,
                {1, 1, out_size, 1});
            ...
            patch_indices =
                dml::Tile(patch_indices, {1, 1, 1, window_sizes[i]});

            // Indices relative to the patch (first element of the patch is at
            // index 0, and last element is at index ksize - 1)
            auto relative_element_indices = dml::Sequence<uint32_t>(
                scope,
                0,
                element_step,
                {1, 1, 1, window_sizes[i]});

            relative_element_indices = dml::Reinterpret(
                relative_element_indices,
                {1, 1, out_size, window_sizes[i]},
    ...
    auto gather_indices = dml::Reinterpret(
        broadcasted_global_indices,
        gathered_sizes,
    ...
    auto gathered_reshaped =
        dml::Reinterpret(gathered, perm_sizes, perm_strides);
