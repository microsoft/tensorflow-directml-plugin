...
struct LRNAttributes
{
    explicit LRNAttributes(OpKernelConstruction* ctx)
    {
        ...
        OP_REQUIRES(
            ctx,
            depth_radius < std::numeric_limits<int>::max(),
...
class LRNInitHelper : public InitializationHelper
{
  public:
    ...
    int64_t GetDepthRadius() const { return attr_->depth_radius; }
...
class DmlLRNKernel : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        // DML divides Alpha by LocalSize, but tensorflow's Alpha already
        // contains that division
...
class LRNGradInitHelper : public InitializationHelper
{
  public:
        ...
        : attr_(attr)
    {
        ...
        OP_REQUIRES(
            context,
            in_grads.dims() == 4 && in_image.dims() == 4,
        ...
        OP_REQUIRES(
            ...
            in_image.dim_size(0) == batch && in_image.dim_size(1) == rows &&
                ...
                out_image.dim_size(0) == batch &&
                out_image.dim_size(1) == rows &&
...
void RegisterLRN()
{
    ...
    RegisterWithTypes<K, ops::LRN::Attribute::T, TF_FLOAT, TF_HALF>();
}

void RegisterLRNGrad()
{
    using K = KernelDefinition<
        ops::LRNGrad,
        DmlKernelWrapper<DmlLRNGradKernel, GetOutputShapeAsInputShapeHelper>>;

    RegisterWithTypes<K, ops::LRNGrad::Attribute::T, TF_FLOAT, TF_HALF>();
}
