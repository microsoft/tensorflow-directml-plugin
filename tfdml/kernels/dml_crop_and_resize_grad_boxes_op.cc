  ...
  public:
    struct Attributes
    {
        explicit Attributes(OpKernelConstruction* ctx)
        {
            ...
            OP_REQUIRES(
                ctx,
                method_attr == "bilinear",
                errors::InvalidArgument(
                    "method must be 'bilinear'",
                    method_attr));
    ...
    CropAndResizeGradBoxesInitHelper(
        OpKernelContext* ctx,
...
class DmlCropAndResizeGradBoxesKernel : public DmlKernel
{
  public:
    ...
    explicit DmlCropAndResizeGradBoxesKernel(
        ...
        const InitHelper* init_helper)
    {
        ...
        // The result will be in order {x1, y1, x2, y2} as per DML's
        // specification, so we need to reverse them like we did for the input
        // ROIs
        auto result = dml::Reinterpret(
            results.outputROIGradient,
            {1, roi_sizes[2], 2, 2},
            {});
...
void RegisterKernels_CropAndResizeGradBoxes()
{
    using K = KernelDefinition<
        ops::CropAndResizeGradBoxes,
        DmlKernelWrapper<
            DmlCropAndResizeGradBoxesKernel,
            CropAndResizeGradBoxesShapeHelper>>;

    RegisterWithTypes<
        K,
        ops::CropAndResizeGradBoxes::Attribute::T,
        TF_FLOAT,
        TF_HALF>();
}
