...
class MirrorPadGradInitHelper : public InitializationHelper
{
  public:
        ...
        : offset_(attr->offset)
    {
        ...
        OP_REQUIRES(
            ctx,
            in0.dims() <= kNcdhwDimensionCount,
            errors::Unimplemented(
                "DML doesn't support more than 5D for MirrorPadGrad, but "
                "found ",
                in0.dims()));
        ...
        {
            const Tpadding before =
                paddings(d, 0); // Pad before existing elements.
            const Tpadding after =
                paddings(d, 1); // Pad after existing elements.
            OP_REQUIRES(
                ctx,
                before >= 0 && after >= 0,
                errors::InvalidArgument(
            ...
            { // SYMMETRIC mode.
                OP_REQUIRES(
                    ctx,
                    before <= out_size && after <= out_size,
                    errors::InvalidArgument(
...
class MirrorPadGradShapeHelper : public ShapeHelper
{
  public:
    ...
    {
        auto init_helper =
            static_cast<const MirrorPadGradInitHelper<Tpadding>*>(
...
{
  public:
    using InitHelper = MirrorPadGradInitHelper<Tpadding>;

    explicit DmlMirrorPadGradKernel(
        ...
        const InitHelper* init_helper)
    {
        ...
        absl::InlinedVector<int32_t, 4> slice_strides(
            dml_input_shape.dims(),
            1);

        // Perform the slicing logic twice for every dimension (for before and
        // after paddings)
        for (int i = 0; i < dml_input_shape.dims(); ++i)
        {
            for (int pad_index = 0; pad_index < 2; ++pad_index)
            {
                ...
                if (padding > 0)
                {
                    ...
                    auto slice =
                        dml::Slice(result, offsets, slice_sizes, slice_strides);

                    uint32_t needed_padding = sizes[i] - padding * 2;

                    // Pad the slice to make it the same size as the rest of the
                    // tensor, which allows for elementwise add to be performed.
                    if (needed_padding != 0)
                    {
                        ...
                        pad_padding[1 - pad_index][i] =
                            init_helper->GetOffset();
                        slice = dml::Padding(
                            slice,
                    ...
                    // Reverse the values on the axis' direction to correctly
                    // fold them on the non-padding part of the tensor
                    ...
                    dml::TensorDesc::Dimensions sliced_result_offsets(
                        sizes.size(),
                        0);
                    uint32_t sliced_result_offset =
                        pad_index == 0 ? padding : 0;
...
using K = typename KernelDefinition<
    Op,
    DmlKernelWrapper<
        DmlMirrorPadGradKernel<TPadding>,
        MirrorPadGradShapeHelper<TPadding>>>::
