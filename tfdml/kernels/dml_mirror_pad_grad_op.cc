...
class DmlMirrorPadGradKernel : public DmlKernel {
 public:
  ...
  explicit DmlMirrorPadGradKernel(DmlKernelConstruction* ctx,
      ...
      for (int pad_index = 0; pad_index < 2; ++pad_index) {
        ...
        if (padding > 0) {
          ...
          if (slice.GetOutputDesc().sizes[i] > 1) {
            ...
            auto seq_lengths = dml::FillValueConstant(scope, seq_lengths_sizes,
                                                      DML_TENSOR_DATA_TYPE_UINT32, seq_lengths_value);
