...
class MatrixBandPartInitHelper : public InitializationHelper
{
  public:
    ...
    MatrixBandPartInitHelper(
        OpKernelContext* ctx,
        std::shared_ptr<const Attributes> attr)
    {
        const Tensor& input = ctx->input(0);
        const TensorShape& input_shape = input.shape();
        const int64_t height = input.dim_size(input.dims() - 2);
        const int64_t width = input.dim_size(input.dims() - 1);
        ...
        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsScalar(num_lower_in.shape()),
            errors::InvalidArgument(
                "num_lower must be scalar, got shape ",
        ...
        auto as_int64_scalar = [](const Tensor& tensor) -> int64_t
        {
            ...
            {
                return tensor.base<int32_t>()[0];
            }
            else
            {
                return tensor.base<int64_t>()[0];
            }
        };
        ...
        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsScalar(num_upper_in.shape()),
            errors::InvalidArgument(
                "num_upper must be scalar, got shape ",
        ...
        OP_REQUIRES(
            ...
            num_upper <= width,
            errors::InvalidArgument(
                "num_upper must be negative or less or "
        ...
        // width, they give the same result as num_lower == height - 1 and
        // num_upper
        // == width - 1
        num_lower_ =
            num_lower < 0 ? height - 1 : std::min(num_lower, height - 1);
        num_upper_ = num_upper < 0 ? width - 1 : std::min(num_upper, width - 1);
    }

    int64_t GetNumLower() const { return num_lower_; }
    int64_t GetNumUpper() const { return num_upper_; }
...
class DmlMatrixBandPartKernel : public DmlKernel
{
  public:
    ...
    DmlMatrixBandPartKernel(
        DmlKernelConstruction* ctx,
        const InitHelper* init_helper)
    {
        const TensorShape input_shape = ctx->GetInputTensorShape(0);
        const int64_t height = input_shape.dim_size(input_shape.dims() - 2);
        const int64_t width = input_shape.dim_size(input_shape.dims() - 1);
        const int64_t batch_size = input_shape.num_elements() / height / width;
        ...
        const int64_t num_lower = init_helper->GetNumLower();
        const int64_t num_upper = init_helper->GetNumUpper();
        const bool is_identity =
            num_lower == height - 1 && num_upper == width - 1;
        ...
        auto col_indices = dml::Sequence<int32_t>(
            ...
            0,
            1,
            {1, 1, 1, static_cast<uint32_t>(width)});

        col_indices = dml::Reinterpret(
            col_indices,
        ...
        auto row_indices = dml::Sequence<int32_t>(
            ...
            0,
            1,
            {1, 1, static_cast<uint32_t>(height), 1});

        row_indices = dml::Reinterpret(
            row_indices,
            input.GetOutputDesc().sizes,
            dml::TensorDesc::Dimensions({0, 0, 1, 0}));

        auto width_tensor = dml::ScalarTensor<int32_t>(
            scope,
            width,
            input.GetOutputDesc().sizes);
        auto int_zero =
...
static void RegisterMatrixBandPart()
{
    using K = KernelDefinition<
        ...
        template WithHostMemoryArguments<
            ops::MatrixBandPart::Argument::num_lower>::
            template WithHostMemoryArguments<
                ops::MatrixBandPart::Argument::num_upper>;

    RegisterWithTypes<
        K,
        ops::MatrixBandPart::Attribute::T,
        TF_FLOAT,
        TF_HALF,
        TF_BOOL>();
}

static void RegisterBatchMatrixBandPart()
{
    using K = KernelDefinition<
        ops::BatchMatrixBandPart,
        DmlKernelWrapper<
            DmlMatrixBandPartKernel,
            GetOutputShapeAsInputShapeHelper>>::
        template WithHostMemoryArguments<
            ops::BatchMatrixBandPart::Argument::num_lower>::
            template WithHostMemoryArguments<
                ops::BatchMatrixBandPart::Argument::num_upper>;

    RegisterWithTypes<
        K,
        ops::BatchMatrixBandPart::Attribute::T,
        TF_FLOAT,
        TF_HALF>();
}
...
{
    RegisterMatrixBandPart();
    RegisterBatchMatrixBandPart();
}
