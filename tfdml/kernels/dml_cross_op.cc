        ...
        std::shared_ptr<const Attributes> attr)
    {
        ...
        OP_REQUIRES(
            ...
            errors::InvalidArgument(
                "Both inputs must be of same shape: ",
...
class DmlCrossKernel : public DmlKernel
{
  public:
    ...
    DmlCrossKernel(DmlKernelConstruction* ctx, const InitHelper* init_helper)
    {
        ...
        first_input.desc = DmlTensorDesc::Create(
        ...
        auto indices = dml::Sequence<int32_t>(scope, -2, 1, {1, 1, 1, 3});

        indices = dml::Reinterpret(
            indices,
            input1.GetOutputDesc().sizes,
        ...
        const bool is_low_precision_int = dtype == TF_UINT16 ||
                                          dtype == TF_INT16 ||
                                          dtype == TF_UINT8 || dtype == TF_INT8;

        // DML doesn't support less than 32bits precision for multiply and
        // subtract
