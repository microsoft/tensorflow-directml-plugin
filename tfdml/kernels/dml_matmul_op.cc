...
class MatMulInitHelper : public InitializationHelper
{
  public:
        ...
        : attr_(attr)
    {
        ...
        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsMatrix(a_shape),
            errors::InvalidArgument(
                "In[0] is not a matrix. Instead it has shape ",
                a_shape.DebugString()));
        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsMatrix(b_shape),
            errors::InvalidArgument(
...
class BaseBatchMatMulInitHelper : public InitializationHelper
{
  public:
    ...
    BaseBatchMatMulInitHelper(
        ...
        std::shared_ptr<const Attributes> attr,
        std::function<
            void(OpKernelContext*, const TensorShape&, const TensorShape&)>
        ...
        : attr_(attr)
    {
        ...
        BCast batches_bcast(
            BCast::FromShape(in0_batches_shape),
            BCast::FromShape(in1_batches_shape));

        OP_REQUIRES(
            ctx,
            batches_bcast.IsValid(),
            errors::InvalidArgument(
        ...
        OP_REQUIRES(
            ctx,
            collapsed_output_shape_.dims() <= 8,
            errors::InvalidArgument(
...
class BatchMatMulInitHelper : public BaseBatchMatMulInitHelper
{
  ...
  private:
    static void ValidateInputTensors(
        OpKernelContext* ctx,
        const TensorShape& in0_shape,
        const TensorShape& in1_shape)
    {
        // Disallow broadcasting support. Ensure that all batch dimensions of
        // the input tensors match.
        OP_REQUIRES(
            ctx,
            in0_shape.dims() == in1_shape.dims(),
            errors::InvalidArgument(
        ...
        for (int i = 0; i < ndims - 2; ++i)
        {
            OP_REQUIRES(
                ctx,
                in0_shape.dim_size(i) == in1_shape.dim_size(i),
                errors::InvalidArgument(
                    "In[0].dim(",
                    i,
                    ") and In[1].dim(",
  ...
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        if (a_tensor_sizes.size() > kNchwDimensionCount)
        {
            ...
            a_tensor =
                dml::Reinterpret(a_tensor, collapsed_sizes, absl::nullopt);
        }
        ...
        if (b_tensor_sizes.size() > kNchwDimensionCount)
        {
            ...
            b_tensor =
                dml::Reinterpret(b_tensor, collapsed_sizes, absl::nullopt);
        }
