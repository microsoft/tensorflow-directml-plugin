 ...
 public:
  ...
  explicit DmlBiasAddKernel(DmlKernelConstruction* ctx,
    ...
    if (data_format == FORMAT_NCHW &&
      ...
      const int64_t missing_dims = kNchwDimensionCount - input_shape.dims();
 ...
 public:
  ...
  BiasAddGradInitHelper(OpKernelContext* ctx,
    ...
    OP_REQUIRES(
            ...
            errors::InvalidArgument("BiasGrad requires tensor size <= int32_t max"));
    ...
    if (attr->data_format == FORMAT_NHWC) {
      ...
      channel_ = static_cast<int32_t>(output_backprop_shape.dim_size(channel_dim));
      for (int32_t i = 0; i < channel_dim; i++) {
        batch_ *= static_cast<int32_t>(output_backprop_shape.dim_size(i));
      }
      ...
      if (output_backprop_shape.dims() > 3) {
        width_ = static_cast<int32_t>(output_backprop_shape.dim_size(3));
      }
      if (output_backprop_shape.dims() > 4) {
        depth_ = static_cast<int32_t>(output_backprop_shape.dim_size(4));
      }
    }
  }
...
static void RegisterBiasAddGrad()
{
    using K = KernelDefinition<
        ops::BiasAddGrad,
        DmlKernelWrapper<
            DmlBiasAddGradKernel,
            BiasAddGradShapeHelper>>;
