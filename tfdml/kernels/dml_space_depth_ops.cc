  ...
  public:
    struct Attributes
    {
        explicit Attributes(OpKernelConstruction* ctx)
        {
            ...
            OP_REQUIRES(
                ctx,
                data_format == FORMAT_NHWC || data_format == FORMAT_NCHW,
            ...
            OP_REQUIRES(
                ...
                block_size > 1,
                errors::InvalidArgument(
                    "Block size should be > 1, but was: ",
                    block_size));
        }
        ...
        : attr_(attr)
    {
        ...
        OP_REQUIRES(
            ...
            input_shape.dims() == 4,
            errors::InvalidArgument(
                "Input rank should be 4 instead of ",
                input_shape.dims()));

        batch_size_ =
            input_shape.dim_size(GetTensorDimIndex<kNchwSpatialDimensionCount>(
                attr->data_format,
                'N'));
        input_height_ =
            input_shape.dim_size(GetTensorDimIndex<kNchwSpatialDimensionCount>(
        ...
        {
            CHECK(operator_type == DML_OPERATOR_SPACE_TO_DEPTH);
...
class SpaceDepthShapeHelper : public ShapeHelper
{
  public:
    ...
    {
        auto init_helper = static_cast<const InitHelper<operator_type>*>(
        ...
        {
            CHECK(operator_type == DML_OPERATOR_SPACE_TO_DEPTH);
            // The 'spatial' block of size block_size X block_size will be moved
            // to depth.
...
class DmlSpaceDepthKernel : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        auto layout = GetDmlTensorLayout(
...
static void RegisterSpaceToDepth()
{
    using K = KernelDefinition<
        ...
        DmlKernelWrapper<
            ...
            SpaceDepthShapeHelper<DML_OPERATOR_SPACE_TO_DEPTH>>>;

    RegisterWithTypes<
        K,
        ops::SpaceToDepth::Attribute::T,
        TF_FLOAT,
        TF_HALF,
        TF_UINT8>();
}
...
static void RegisterDepthToSpace()
{
    using K = KernelDefinition<
        ...
        DmlKernelWrapper<
            ...
            SpaceDepthShapeHelper<DML_OPERATOR_DEPTH_TO_SPACE>>>;
...
{
    RegisterSpaceToDepth();
    RegisterDepthToSpace();
}
