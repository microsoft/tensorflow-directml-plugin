...
#include "tfdml/kernels/dml_extract_patches_helpers.h"
...
static inline void ParseAttributeVec5(
    OpKernelConstruction* ctx,
    const std::string& attr_name,
    /*out*/ absl::Span<uint32_t> attr)
{
    std::vector<int32_t> int32_attr;
    OP_REQUIRES_OK(ctx, ctx->GetAttr(attr_name.c_str(), &int32_attr));
    OP_REQUIRES(
        ctx,
        (int32_attr)[0] == 1 && (int32_attr)[4] == 1,
        errors::Unimplemented("Only support ", attr_name, " across space."));
    OP_REQUIRES(
        ctx,
        (int32_attr)[1] >= 1 && (int32_attr)[2] >= 1 && (int32_attr)[3] >= 1,
    ...
    std::transform(
        ...
        int32_attr.end(),
        attr.begin(),
        [](int32_t val) { return static_cast<uint32_t>(val); });
}
  ...
  public:
    struct Attributes
    {
        explicit Attributes(OpKernelConstruction* ctx)
        {
            ...
            // TODO: Support rates if the CPU/GPU kernels ever support them
            // (still unsupported as of TensorFlow 2.8.0). For now, we set them
            // to 1 to get the same results as the CPU and the GPU.
            rates[0] = 1;
            rates[1] = 1;
            rates[2] = 1;
            rates[3] = 1;
        ...
        : attr_(std::move(attr))
    {
        ...
        int64_t out_planes, out_rows, out_cols;
        int64_t planes_padding_before, rows_padding_before, cols_padding_before;
        int64_t planes_padding_after, rows_padding_after, cols_padding_after;
        ...
        OP_REQUIRES_OK(
            ...
            GetWindowedOutputSizeVerbose(
                ...
                attr_->padding,
                &out_planes,
                &planes_padding_before,
                &planes_padding_after));
        OP_REQUIRES_OK(
            ctx,
            GetWindowedOutputSizeVerbose(
                in_rows,
    ...
    absl::Span<const uint32_t> GetStartPadding() const
    {
    ...
    absl::Span<const uint32_t> GetWindowStrides() const
    {
        return attr_->strides;
    }
...
class DmlExtractVolumePatchesKernel : public DmlKernel
{
  public:
        ...
        const InitHelper* init_helper)
    {
        ...
        auto result = dml::ExtractPatches(
            scope,
            input,
            window_sizes,
            window_strides,
...
void RegisterKernels_ExtractVolumePatches()
{
    using K = KernelDefinition<
        ops::ExtractVolumePatches,
        DmlKernelWrapper<
            DmlExtractVolumePatchesKernel,
            ExtractVolumePatchesShapeHelper>>;

    RegisterWithTypes<
        K,
        ops::ExtractVolumePatches::Attribute::T,
        TF_FLOAT,
        TF_HALF>();
}
