...
class DmlTensorDesc
{
  public:
    ...
    //
    // data_type:
    //   The data type of the tensor. This must be one of the data types
    //   supported by DML.
    //
    // shape:
    ...
    //   method assumes the desired canonical layout is NC[D]HW, which is true
    //   for most DML operators that care about layout. For example, a `shape`
    //   of {2, 3, 4} and tensor_layout of {N, W, C} means that N=2, W=3, and
    //   C=4, and will produce a 4D DML tensor desc of sizes {2, 4, 1, 3}. If
    //   one of the dimensions is 'D', then this method returns a 5D tensor
    //   desc, otherwise it returns a 4D tensor desc. This parameter is
    //   optional. If not supplied, no shuffling of dimensions will occur.
    //
    // guaranteed_base_offset_alignment:
    //   The value of DML_TENSOR_DESC::GuaranteedBaseOffsetAlignment to set, or
    //   0 for the default.
    ...
    // dimensions (inclusive), while the tensor_layout overload only accepts up
    // to 5 dimensions.
    static DmlTensorDesc Create(
        TF_DataType data_type,
        const TensorShape& shape,
    ...
    TF_DataType GetTfDataType() const { return tf_tensor_type_; }
    ...
    absl::Span<const uint32_t> GetSizes() const
    {
        return absl::MakeSpan(
            sizes_,
            sizes_ + buffer_tensor_desc_.DimensionCount);
    }
