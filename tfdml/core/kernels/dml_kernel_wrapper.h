...
class DmlKernelWrapper : public DmlKernelWrapperBase
{
  ...
  protected:
    ...
    {
        // If the cache policy is "Never", the kernel wrapper should simply
        // create the kernel directly instead of delegating to the kernel
        // manager
        ...
        return kernel_manager.CreateCachedKernel<TKernel>(
            ctx,
            key,
            static_cast<const typename TKernel::InitHelper*>(
                initialized_helper));
    }
    ...
    {
        // If the cache policy is "Never", the kernel wrapper should never try
        // to retrieved a cached kernel
        assert(cache_policy != DmlKernelCachePolicy::Never);
