...
class GatherNdInitHelper : public InitializationHelper
{
  public:
        ...
        std::shared_ptr<const Attributes> attr)
    {
        ...
        OP_REQUIRES(
            ctx,
            TensorShapeUtils::IsVectorOrHigher(indices.shape()),
            errors::InvalidArgument("indices must be at least a vector"));
        OP_REQUIRES(
            ctx,
            indices.dim_size(indices.dims() - 1) <= params.dims(),
        ...
        OP_REQUIRES(
            ctx,
            N_big <= std::numeric_limits<int>::max(),
            errors::InvalidArgument(
        ...
        OP_REQUIRES(
            ctx,
            params.NumElements() <= std::numeric_limits<TIndex>::max(),
            errors::InvalidArgument(
                "params.NumElements() too large for ",
                DataTypeString(DataTypeToEnum<TIndex>()),
                " indexing: ",
                params.NumElements(),
                " > ",
        ...
        OP_REQUIRES(
            ctx,
            slice_size_big <= std::numeric_limits<TIndex>::max(),
            errors::InvalidArgument(
        ...
        absl::Span<const TensorShape> output_shapes) const final
    {
        ...
        int64_t last_indices_dim = indices.dim_size(indices.dims() - 1);
...
template <typename TIndex> class GatherNdShapeHelper : public ShapeHelper
{
  public:
    ...
    {
        auto init_helper = static_cast<const GatherNdInitHelper<TIndex>*>(
...
class DmlGatherNdKernel : public DmlKernel
{
  public:
    ...
    DmlGatherNdKernel(DmlKernelConstruction* ctx, const InitHelper* init_helper)
    {
        ...
        if (last_indices_dim == 0)
        {
            ...
            auto result = dml::Tile(
                params,
                {static_cast<uint32_t>(indices_leading_dims), 1, 1, 1});
        ...
        else
        {
            ...
            auto result = dml::GatherND(
                params,
                indices,
                flat_params_shape.dims(),
                flat_indices_shape.dims(),
                0);
    ...
    StatusOr<DmlGpuEvent> Compute(DmlKernelContext* ctx) const override
    {
        ...
        absl::InlinedVector<absl::optional<DML_BUFFER_BINDING>, 2>
            input_bindings;
...
#define DML_REGISTER_KERNELS(type)                                             \
    ...
    REGISTER_KERNEL_BUILDER(                                                   \
        ...
        DmlGatherNdWrapper<int64_t, GatherNdHostInputIndices>)
...
#define DML_REGISTER_KERNELS(type)                                             \
    REGISTER_KERNEL_BUILDER(                                                   \
        Name("ResourceGatherNd")                                               \
            ...
            .TypeConstraint<int32_t>("Tindices"),                              \
        DmlGatherNdWrapper<int32_t, ResourceGatherNdHostInputIndices>)         \
    REGISTER_KERNEL_BUILDER(                                                   \
        Name("ResourceGatherNd")                                               \
            .Device(DEVICE_DML)                                                \
            .HostMemory("resource")                                            \
            .TypeConstraint<type>("dtype")                                     \
            .TypeConstraint<int64_t>("Tindices"),                              \
        DmlGatherNdWrapper<int64_t, ResourceGatherNdHostInputIndices>)
