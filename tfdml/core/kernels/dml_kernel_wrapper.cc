...
DmlKernelWrapperBase::DmlKernelWrapperBase(DmlKernelCachePolicy cache_policy,
                                           const char* op_type_string,
                                           const char* op_name)
    : OpKernel(op_type_string, op_name), cache_policy_(cache_policy) {}
  ...
  if (kernel) {
    ...
    if (shared_helper->IsNoOpKernel(ctx, output_shapes)) {
      ...
      for (int i = 0; i < ctx->num_outputs(); ++i) {
        ...
        Tensor output_tensor = status_or_output_tensor.ConsumeValueOrDie();

        // If the tensor is nonempty, fill it with zero's
        if (output_tensor.NumElements() != 0) {
          DMLDeviceContext* device_context = dml_device->GetDeviceContext();
    ...
    DmlKernelConstruction dml_construction(dml_device, ctx, output_shapes,
                                           shared_helper);
