...
class DmlExecutionContext
{
  public:
    ...
    DmlGpuEvent CopyBufferRegionRaw(
    ...
    // calling this method.
    DmlGpuEvent FillBufferWithPatternRaw(
        ID3D12Resource* dst,
        uint64_t dst_offset,
        uint64_t dst_size_in_bytes,
        absl::Span<const uint8_t> value);

    inline DmlGpuEvent FillBufferWithPattern(
        const D3D12BufferRegion& dst,
        absl::Span<const uint8_t> value)
    {
        return FillBufferWithPatternRaw(
            dst.ResourceInUavState(),
            dst.Offset(),
            dst.SizeInBytes(),
            value);
    }
