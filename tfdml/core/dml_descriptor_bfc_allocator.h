...
#include "tfdml/core/util/bfc_allocator.h"
...
// Although DmlAllocator can be wrapped by the framework's BFCAllocator, we
// can't used the same strategy here since we're allocating descriptors, which
// is an implementation detail of DML. For this reason, we copied the BFC
// allocator classes from the tfdml 1.15 code just to use it here.
class DmlDescriptorAllocator : public BFCAllocator
{
    ...
    class SubAllocatorWrapper final : public SubAllocator
    {
      public:
