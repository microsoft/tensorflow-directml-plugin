...
class DmlDeviceCache
{
  public:
    ...
    // Parts of the grappler will look up physical hardware properties using a
    // TF device specification (e.g. '/device:DML:0'). However, TF device IDs
    // are not directly equivalent to the adapter index. Device IDs to adapter
    // index mappings are influenced by two separate mechanisms:
    //
    ...
    // adapter index, but there is a problem: the visible_device_list is scoped
    // to a TF session, and a single process can have multiple sessions. This
    // would be manageable if sessions had unique IDs and the grappler code
    // propagated this ID to its hardware lookup functions, but alas this is not
    // the case. Consequently, these helpers are on a process-wide singleton and
    // they may fail if multiple sessions use different visible device lists
    // (same as GPU helpers).
    //
  ...
  private:
    ...
    std::unordered_map<int, uint32_t> device_id_to_adapter_index_map_;
