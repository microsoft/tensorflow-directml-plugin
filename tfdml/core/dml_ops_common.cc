...
static uint32_t GetDmlInputTensorCount(
...
static uint32_t GetDmlOutputTensorCount(
    DmlKernelConstruction* ctx,
    const DmlKernelParams& params)
{
    ...
    const absl::optional<TensorShape>& tensor_shape)
{
    ...
    TF_DataType data_type = ctx->GetInputDataType(kernel_index);
    ...
    const absl::optional<TensorShape>& tensor_shape)
{
    ...
    TF_DataType data_type = ctx->GetOutputDataType(kernel_index);
    ...
    const DML_OPERATOR_DESC& op_desc)
{
    ...
    DML_CHECK_SUCCEEDED(dml_device->CompileOperator(
        op.Get(),
        execution_flags,
    ...
    IDMLCompiledOperator* compiled_op)
{
    ...
    if (exec_binding_props.PersistentResourceSize != 0)
    {
        TF_VLog(
            2,
            "Allocating %s persistent resource for kernel %s",
    ...
    absl::optional<DmlBuffer> temp_resource;
    ...
    // Enqueue an event to ensure that the relevant initialization state lives
    // at least until the operation completes execution on the GPU.
    auto on_initialize_completed =
        [p = std::move(initializer), p2 = std::move(descriptor_range)]() mutable
    {
    ...
    absl::Span<const absl::optional<DML_BUFFER_BINDING>> output_bindings) const
{
    ...
    if (temporary_resource_size > 0)
    {
        // Allocate a temporary buffer and keep a use on it until the end of
        // this method. The buffer resource will still be alive (managed by the
        // pool); freeing allows the resource to be shared with other operators,
...
{
    absl::InlinedVector<D3D12BufferRegion, 8> input_buffers(
        input_descs_.size());
...
DML_SCALAR_UNION ScalarUnion(double value, DML_TENSOR_DATA_TYPE data_type)
{
    ...
    case DML_TENSOR_DATA_TYPE_FLOAT16: {
        ...
        const BYTE* float16_bytes =
            reinterpret_cast<const BYTE*>(&float16_value);
        std::copy(
            float16_bytes,
            float16_bytes + sizeof(float16_value),
            scalar.Bytes);
    }
    break;

    default: DML_CHECK_SUCCEEDED(E_INVALIDARG); break;
    }
