...
class DmlDescriptorHeap
{
  public:
    ...
    // Reserves descriptors from the end of the heap. Returns nullopt if there
    // is no space left in the heap.
...
class DmlDescriptorPool
{
  public:
    ...
    // DmlDescriptorPool class. The caller must supply a DmlGpuEvent that
    // informs the pool when the reserved descriptors are no longer required.
