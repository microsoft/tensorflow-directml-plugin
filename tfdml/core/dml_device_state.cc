    ...
    const DmlAdapter& adapter)
{
        ...
        SUCCEEDED(d3d_device->QueryInterface(IID_PPV_ARGS(&info_queue))))
    {
        ...
        // Manually load DirectML.Debug.dll prior to creating the DML device, as
        // a way to override the DLL search path in case TF_DIRECTML_PATH is
        // set. We just need the DLL loaded into memory which is why we can
        // throw away the result of this function.
        DmlCachedDsoLoader::GetDirectMLDebugDsoHandle();
    }
#endif
    ...
    // Default to using compute queues for AMD since it seems to mitigate TDRs
    // and improve performance
    const bool use_compute_queue_default = adapter.VendorID() == VendorID::kAmd;

    bool use_compute_queue;
    Status s = ReadBoolFromEnvVar(
        "TF_DIRECTML_USE_COMPUTE_QUEUE",
        use_compute_queue_default,
        &use_compute_queue);
