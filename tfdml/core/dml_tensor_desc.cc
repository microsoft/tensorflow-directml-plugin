    ...
    uint64_t end_padding_in_bytes)
{
    ...
    buffer_tensor_desc_.TotalTensorSizeInBytes =
        DMLCalcBufferTensorSize(
            buffer_tensor_desc_.DataType,
            buffer_tensor_desc_.DimensionCount,
            sizes_,
    ...
    uint32_t guaranteed_base_offset_alignment)
{
    ...
    const auto& non_broadcast_dimensions =
        NarrowTensorShape(non_broadcast_shape);
    ...
    TF_DataType data_type)
{
    ...
    // broadcasting. Broadcasting stretches any dimensions with a single
    // element.
    //
    // e.g. physical [2,1,4]
    //       desired [2,3,4]
    ...
    // `tensor_layout`; these are later swizzled into the canonical NCHW order
    // as required by DML.
    ...
    for (uint32_t i = 0; i < rank; ++i)
    {
        ...
        // If the non-broadcast shape has fewer dimensions, the missing
        // dimensions are equivalent to having a size of 1
    ...
    uint32_t guaranteed_base_offset_alignment)
{
    ...
    assert(AsSet(tensor_layout).size() == tensor_layout.size());
    ...
    // DML only supports two dimension counts: 4 and 5. A dimension count of 5
    // is used only if one of the dimensions is D (for depth)
    const bool has_depth_dim = std::find(
                                   tensor_layout.begin(),
                                   tensor_layout.end(),
                                   DmlTensorAxis::D) != tensor_layout.end();
    ...
    absl::InlinedVector<uint32_t, DML_TENSOR_DIMENSION_COUNT_MAX> dml_strides(
        dml_dimension_count,
        default_dimension_stride);

    assert(dimensions.size() == rank);
    assert(strides.size() == rank);
    assert(tensor_layout.size() == rank);

    // Walk through the sizes and strides and assign them to the correct
    // dimension index for the DML tensor desc
    for (uint32_t i = 0; i < rank; ++i)
    {
        ...
        // The index of a particular dimension (e.g. 'H') can vary depending on
        // the dimension count
        uint32_t dml_dim_index =
            GetDmlDimensionIndex(axis, dml_dimension_count);
...
void DmlTensorDesc::ForceUnsignedDataType()
{
    ...
    default: LogFatal("Can't coerce unknown or non-integral data type");
    }
}
} // namespace tfdml