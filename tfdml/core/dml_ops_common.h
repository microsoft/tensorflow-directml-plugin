...
struct DmlKernelParams
{
    ...
    // corresponding input tensor to the kernel. For example, if the DML
    // operator takes 3 inputs in the following order:
    //   A
    //   B
    //   {null}
    //   C
    ...
    // same number of inputs, and they're in the same order, then this vector
    // can be empty.
    ...
    // Specifies a tensor shape to broadcast/coerce all input tensors to. This
    // is often used to handle the numpy-style broadcasting as required by TF.
    // For example, DML requires that all tensors supplied to ELEMENT_WISE_ADD
    // must have identical sizes and dimension counts. However due to
    // broadcasting, a kernel for Add might receive one input with shape {1, 2},
    // to be added to a tensor with shape {5, 1, 2}. In this case the
    // expectation is that the {1, 2} tensor should be broadcast into a tensor
    // of shape {5, 1, 2}.
    //
    // In the above example, supplying a tensor shape of {5, 1, 2} will cause
    // the {1, 2} tensor's desc to be automatically broadcasted to {5, 1, 2}.
...
class DmlKernel
{
  public:
    ...
    // when the kernel completes execution on the GPU. This method is
    // thread-safe.
    ...
    bool SupportsInPlaceExecution() const
    {
        return supports_in_place_execution_;
    }
  ...
  protected:
    ...
    // required tensor descs. This takes ownership of the DmlKernelTensors so
    // when this method returns, the `tensor_descs` object will be empty. This
    // method must be called exactly once.
    void Initialize(
        DmlKernelConstruction* ctx,
    ...
    // Same as Compute(), except that it allows for custom input/output
    // bindings.
  ...
  private:
    ...
    // this vector can be empty. Note that the vector's index represents the
    // index of the output; not the index of the input.
    //
    // For example, if we need to forward the first input to the first output,
    // we would have a vector with a single element: {0} On the other hand, if
    // we need to forward the second input to the first output, we would have
    // the following vector: {1} Finally, if we have 2 outputs but only the
    // second one is a ref output and needs the second input to be forwarded to
    // it, we would have the following vector: {nullopt, 1}
...
Expression ScalarTensor(Graph& scope, T value, TensorDesc::Dimensions sizes)
{
    ...
    auto scalar = dml::Reinterpret(
        dml::FillValueConstant(
            scope,
            scalar_dims,
            tfdml::TfTensorTypeTraits<T>::dml_type,
            tfdml::TfTensorTypeTraits<T>::ToDmlScalar(value)),
...
template <typename T>
Expression Sequence(Graph& scope, T start, T step, TensorDesc::Dimensions sizes)
{
    auto seq = dml::FillValueSequence(
        scope,
        sizes,
        tfdml::TfTensorTypeTraits<T>::dml_type,
        tfdml::TfTensorTypeTraits<T>::ToDmlScalar(start),
        tfdml::TfTensorTypeTraits<T>::ToDmlScalar(step));
