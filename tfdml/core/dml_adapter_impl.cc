    ...
    /*out*/ std::vector<uint32_t>* adapter_indices)
{
    ...
    const std::vector<std::string> indices_str =
        absl::StrSplit(visible_device_list, ',');
    adapter_indices->clear();
    adapter_indices->reserve(indices_str.size());

    for (const std::string& index_str : indices_str)
    {
        uint32_t adapter_index;
        if (!absl::SimpleAtoi<uint32_t>(index_str, &adapter_index))
        {
            if (skip_invalid)
            {
                break;
    ...
    // Validate no repeats.
    std::set<uint32_t> adapter_index_set(
        adapter_indices->begin(),
        adapter_indices->end());
    ...
    absl::Span<const DmlAdapterImpl> adapters)
{
    ...
    TF_CHECK_OK(ParseVisibleDeviceList(
        dml_visible_devices,
        adapters.size(),
        skip_invalid,
        &visible_device_list));
...
bool DmlAdapterImpl::IsUmaAdapter() const
{
    D3D_FEATURE_LEVEL feature_level =
        IsComputeOnly() ? D3D_FEATURE_LEVEL_1_0_CORE : D3D_FEATURE_LEVEL_11_0;

    ComPtr<ID3D12Device> d3d12_device =
        TryCreateD3d12Device(Get(), feature_level);
...
void DmlAdapterImpl::Initialize(IDXGIAdapter* adapter)
{
    ...
    vendor_id_ = static_cast<tfdml::VendorID>(desc.VendorId);
...
std::vector<DmlAdapterImpl> EnumerateAdapterImpls()
{
    ...
    {
        // Enumerate adapters by performance. This only works in Windows 10
    ...
    else
    {
        ...
        for (uint32_t adapter_index = 0;
             ...
             adapter_index++)
        {
            // We can't assume the ordering of hardware and software adapters,
            // so keep looping. This path should only execute on Windows 10
            // version 1709 or earlier; IDD adapters do not exist when taking
            // this code path
            if (IsSoftwareAdapter(adapter.Get()))
            {
                continue;
...
void DmlAdapterImpl::Initialize(IDXCoreAdapter* adapter)
{
    ...
    DML_CHECK_SUCCEEDED(adapter->GetProperty(
        ...
        sizeof(dedicated_memory_in_bytes_),
        &dedicated_memory_in_bytes_));
