...
DmlReadbackHeap::DmlReadbackHeap(
    ID3D12Device* device,
      ...
      event_queue_(event_queue)
{
    ...
    DML_CHECK_SUCCEEDED(device->CreateFence(
        0,
    ...
    const D3D12BufferRegion& src)
{
    ...
    assert(!dst.empty());
    assert(
        src.ResourceInUavState()->GetDesc().Dimension ==
    ...
    // Allocations from the readback pool are only ever used as copy
    // destinations.
    auto readback_resource = D3D12BufferRegion(
    ...
    auto done_callback = [this, dst, readback_heap, offset_in_chunk, done_event]
    {
        ...
        DML_CHECK_SUCCEEDED(
            readback_heap->Map(0, nullptr, &readback_heap_data));
        readback_heap_data =
