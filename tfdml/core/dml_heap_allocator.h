...
class D3D12HeapAllocator
{
  public:
    ...
    // local video memory fragmentation without requiring lots of heaps. This
    // value can be overridden using the TF_DIRECTML_MAX_HEAP_TILES environment
    // variable.
    ...
    // large allocations. This value can be overridden using the
    ...
    void* Alloc(uint32_t device_id, uint64_t size_in_bytes);
