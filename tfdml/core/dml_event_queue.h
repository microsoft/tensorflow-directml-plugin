  ...
  private:
    ...
    struct SharedState
    {
        ...
        std::condition_variable
            new_event_enqueued; // An event that fires whenever
                                // a new event is added.
        std::multimap<uint64_t, Event> events_by_fence_value;

        // The current fence value that the thread is waiting to be signaled.
        // This value is guaranteed to be <= fence->GetCompletedValue()+1.
