...
void DmlEventQueue::Enqueue(DmlGpuEvent gpu_event, DoneCallback done_callback)
{
    ...
    assert(
        state->fence->GetCompletedValue() + 1 >=
        state->current_awaited_fence_value);

    // If the thread is currently waiting on a fence value less or equal than
    // the supplied gpu_event, it means the fence may or may not be signaled,
    // and therefore should be queued to be processed by the thread. Otherwise,
    // if the fence value is greater, it means it's definitely already been
    // signaled and the callback should be invoked immediately.
    if (state->current_awaited_fence_value <= gpu_event.fence_value)
    {
        // Queue the event and notify the thread.
        state->events_by_fence_value.emplace(
            gpu_event.fence_value,
            Event{std::move(done_callback)});
        state->new_event_enqueued.notify_all();
    }
    else
    {
        // Sanity: all prior events in the queue should have been processed
        // already
        assert(
            state->events_by_fence_value.lower_bound(gpu_event.fence_value) ==
...
/*static*/ void DmlEventQueue::ThreadProc(std::shared_ptr<SharedState> state)
{
    ...
    while (true)
    {
        ...
        if (state->events_by_fence_value.empty())
        {
            ...
            // No need for a loop around the wait() in case of spurious wakeup;
            // just return to the top. This also handles the case where exit is
            // requested.
            continue;
        }

        assert(!state->events_by_fence_value.empty());
        ...
        //
        // Note that because fences are asynchronous, this next_fence_value
        // could become signaled at any time (even immediately). This is okay;
        // it just means that the wait we perform below will return immediately,
        // and the loop will continue.
        uint64_t next_fence_value = state->fence->GetCompletedValue() + 1;

        // Device removal will result in all fences reporting a completed value
        // of UINT64_MAX; GetCompletedValue()+1 will overflow to 0. The
        // intention here is to ensure all outstanding events are signaled, but
        // this queue uses the *next* value (0) to flush events in a batch. If
        // the device is removed we explicitly set the next_fence_value to
        // UINT64_MAX so that all events are flushed then exit this thread.
        ...
        // events, by definition, have had their fence values signaled and can
        // now be processed.
        ...
        {
            assert(it->first < next_fence_value);
        ...
        // If the queue isn't empty then there are still events waiting for a
        // fence value that hasn't been reached. The WaitForSignal below is to
        // avoid a busy wait, but it's not required for correctness.
        // WaitForSignal should NOT be invoked if the queue is already empty
        // since another signal may never come (i.e. no more events are
        // enqueued).
        if (!event_queue_empty)
        {
            // Now wait for the fence to become signaled again. Recall that the
            // choice of next_fence_value ensures this wait will complete no
            // matter what value is signaled.
            DmlGpuEvent next_event{next_fence_value, state->fence};
            next_event.WaitForSignal();

            // We require monotonically increasing fence values; time is not
