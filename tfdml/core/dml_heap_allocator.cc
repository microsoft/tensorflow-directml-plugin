...
static bool GetTilingEnabled(ID3D12Device* device)
{
    ...
    if (force_placed_resources_string != nullptr)
    {
        ...
        else
        {
            TF_Log(
                TF_ERROR,
                "The TF_DIRECTML_FORCE_PLACED_RESOURCES environment variable "
                "is "
                "set but could not be parsed: \"%s\". Valid values are "
                "\"true\" "
                "or \"false\". Using default value of \"false\".",
                force_placed_resources_string);
        }
    }
...
{
    TF_VLog(1, "Tiling enabled = %d", tiling_enabled_);
    TF_VLog(1, "Max heap size in tiles = %llu", max_heap_size_in_tiles_);
}

absl::optional<D3D12HeapAllocator::Allocation> D3D12HeapAllocator::
    TryCreateTiledAllocation(uint64_t size_in_bytes)
{
    ...
    for (int i = 0; i < ABSL_ARRAYSIZE(resources); i++)
    {
        ...
        {
            TF_Log(TF_WARNING, "DML allocator out of memory!");
            return absl::nullopt;
        }
...
absl::optional<D3D12HeapAllocator::Allocation> D3D12HeapAllocator::
    TryCreateUntiledAllocation(uint64_t size_in_bytes)
{
    Allocation allocation = {};
...
void* D3D12HeapAllocator::Alloc(uint64_t size_in_bytes)
{
    ...
    if (!id)
    {
        TF_Log(TF_WARNING, "DML allocator ran out of allocation IDs!");
        return nullptr;
    }

    TF_VLog(
        3,
        "D3D12HeapAllocator: allocating id=%u, %s",
        *id,
        strings::HumanReadableNumBytes(size_in_bytes).c_str());
...
void D3D12HeapAllocator::Free(void* ptr, uint64_t size_in_bytes)
{
    ...
    TF_VLog(
        3,
        "D3D12HeapAllocator: freeing id=%llu, %s",
