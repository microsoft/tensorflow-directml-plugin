...
{
    // Bail if the desired heap creation flags are incompatible with the
    // existing heap.
    ...
    // The caller will need to create a new heap if there is no space left in
    // this one.
...
DmlDescriptorPool::DmlDescriptorPool(
    ID3D12Device* device,
    ...
    D3D12_DESCRIPTOR_HEAP_FLAGS heap_flags)
{
    ...
    {
        auto descriptor_range = heap.TryAllocDescriptors(
            num_descriptors,
            completion_event,
            heap_flags);
...
void DmlDescriptorPool::CreateHeap(
    uint32_t num_descriptors,
    D3D12_DESCRIPTOR_HEAP_FLAGS heap_flags)
{
