...
BCast::BCast(const Vec& sx, const Vec& sy, const bool fewer_dims_optimization)
{
    ...
    else
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            // Output shape.
            State curr = UNKNOWN;
            const int64_t x_i = x[i]; // i-th dimension of x.
            const int64_t y_i = y[i]; // i-th dimension of y.
            int64_t o_i;              // i-th dimension of the output.
            int64_t bx_i;             // i-th broadcast for x.
            int64_t by_i;             // i-th broadcast for y.
            // Invariant:
            ...
            // Reshape/broadcast.
            // Invariant:
            //  result[i] == x_reshape[i] * x_bcast[i] == y_reshape_[i] *
            //  y_bcast_[i]
            if (curr == SAME && x_i == 1)
            {
                // Both side are 1s.
            ...
            {
                // It is a run of the same cases(no broadcast, x broadcast to y,
                // y broadcast to x). We can reshape the input so that fewer
