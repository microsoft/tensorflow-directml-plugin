...
struct AllocationAttributes
{
    ...
    // EXPERIMENTAL: If provided, then evaluates to a timing count such that
    // only a memory chunk whose freed_at_count is at this value or earlier may
    // be returned.
...
class Allocator
{
  public:
    ...
    // REQUIRES: "alignment" is a power of 2.
    virtual void* AllocateRaw(
        size_t alignment,
        size_t num_bytes,
        const AllocationAttributes& allocation_attr)
    {
        // The default behavior is to use the implementation without any
...
struct AllocatorAttributes
{
    ...
    uint32_t value = 0;
    // EXPERIMENTAL: If this is greater than zero, then allocation is delegated
    // to a named special-purpose allocator on the same device.
    int32_t scope_id = 0;
...
// call it directly.
Allocator* cpu_allocator(int numa_node = -1);
