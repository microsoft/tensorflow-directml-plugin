...
static bool SupportsAllDataTypes(const DmlAdapter& adapter,
  ...
  return std::all_of(
      ...
      [&dml_device, &adapter](DML_TENSOR_DATA_TYPE data_type) {
        ...
        if (FAILED(hr)) {
          TF_Log(TF_WARNING,
                 "CheckFeatureSupport (data type = %d) failed for adapter: %s",
                 static_cast<int>(data_type), adapter.Name().c_str());
          return false;
        }
...
static bool SupportsDmlDevice(const DmlAdapter& adapter) {
  ...
  if (!d3d12_device) {
    TF_Log(TF_WARNING, "Could not create Direct3D device for adapter: %s",
           adapter.Name().c_str());
    return false;
  }
  ...
  if (!dml_device) {
    TF_Log(TF_WARNING, "Could not create DirectML device for adapter: %s",
           adapter.Name().c_str());
    return false;
  }
...
static std::vector<DmlAdapter> FilterAdapters() {
  ...
  if (!dml_handle_or.ok()) {
    ...
    if (path) {
      TF_Log(TF_WARNING, "Could not load DirectML. TF_DIRECTML_PATH is set: %s",
             path);
    } else {
...
const DmlDeviceState* DmlDeviceCache::GetOrCreateDeviceState(
  ...
  if (!device_states_[adapter_index]) {
    ...
    TF_Log(TF_INFO, "DirectML: creating device on adapter %u (%s)",
           adapter_index, adapter.Name().c_str());
...
DmlDeviceCache::DmlDeviceCache() : adapters_(FilterAdapters()) {
  ...
  for (size_t i = 0; i < adapters_.size(); ++i) {
    ...
    TF_VLog(1, "    Driver: %u.%u.%u.%u", driver_ver.a, driver_ver.b,
            driver_ver.c, driver_ver.d);
    TF_VLog(1, "    IsComputeOnly: %s",
