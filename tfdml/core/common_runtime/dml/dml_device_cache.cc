...
Status DmlDeviceCache::MapDeviceIdToAdapterIndex(
    int device_id,
    uint32_t adapter_index)
{
    // Try to insert the device_id -> adapter_index mapping. This will fail if
    // the device_id has already been mapped, but that's OK so long as the
    ...
    if (prev_adapter_index && (*prev_adapter_index != adapter_index))
    {
        return errors::AlreadyExists(
            ...
            *prev_adapter_index,
            ...
            "(ConfigProto.gpu_options, for example different "
            "visible_device_list)"
...
Status DmlDeviceCache::GetAdapterIndexFromDeviceId(
    int device_id,
    uint32_t* adapter_index)
{
    {
