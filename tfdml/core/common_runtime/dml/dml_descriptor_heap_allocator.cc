...
void* D3D12DescriptorHeapAllocator::Alloc(uint64_t size_in_descriptors) {
  ...
  if (!id) {
    TF_Log(TF_WARNING, "DML descriptor allocator ran out of allocation IDs!");
    return nullptr;
  }

  TF_VLog(3, "D3D12DescriptorHeapAllocator: allocating id=%u, %llu descriptors",
          *id, size_in_descriptors);
...
void D3D12DescriptorHeapAllocator::Free(void* ptr,
  ...
  TF_VLog(3, "D3D12DescriptorHeapAllocator: freeing id=%llu, %llu descriptors",
