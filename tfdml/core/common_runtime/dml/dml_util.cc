...
Microsoft::WRL::ComPtr<TDeviceOrFactory> DxExportHelper(
  ...
  if (!get_symbol_status.ok()) {
    switch (module_error_handling) {
      case DxCallErrorHandling::Warning:
        TF_Log(TF_WARNING, "Could not find symbol '%s' in '%s' module.",
               function_name, module_name);
        break;
      case DxCallErrorHandling::Fatal:
        LogFatal("Could not find symbol '%s' in '%s' module.", function_name,
                 module_name);
        break;
      case DxCallErrorHandling::Silent:
        // Silent doesn't log anything
  ...
  if (FAILED(hr)) {
    switch (call_error_handling) {
      case DxCallErrorHandling::Warning:
        TF_Log(TF_WARNING, "'%s' failed with HRESULT %#010x", function_name,
               hr);
        break;
      case DxCallErrorHandling::Fatal:
...
Microsoft::WRL::ComPtr<IDMLDevice> TryCreateDmlDevice(
  ...
  if (!module_handle.ok() && getenv("TF_DIRECTML_PATH")) {
    TF_Log(TF_WARNING, "Could not find DirectML with TF_DIRECTML_PATH set.");
  }
...
TF_DataType GetTfDataTypeFromDmlDataType(DML_TENSOR_DATA_TYPE type) {
  switch (type) {
    ...
    case DML_TENSOR_DATA_TYPE_INT8:
      return TF_INT8;
    default:
      LogFatal("Invalid or unsupported data type.");
  }
}
...
DmlTensorLayout GetDmlTensorLayout(TensorFormat format, uint32_t rank) {
  ...
  switch (format) {
    ...
    case FORMAT_NCHW:
      ...
      break;
    default:
      LogFatal("Unsupported tensor layout");
  }
...
D3D12BufferRegion CreateBufferForDeviceMemory(const DmlDevice* device,
  ...
  auto region = allocator->CreateBufferRegion(data->opaque, size_in_bytes);
