...
Status DMLDeviceContext::CopyCPUTensorToDevice(DmlDevice* device,
...
void DMLDeviceContext::CopyTensorInSameDevice(DmlDevice* device,
...
Status DMLDeviceContext::CopyDeviceTensorToCPU(DmlDevice* device,
...
Status DMLDeviceContext::CopyCPUMemoryToDevice(
    DmlDevice* device, const void* cpu_memory,
    SP_DeviceMemoryBase* device_memory, uint64_t size_in_bytes) const {
  if (size_in_bytes == 0) {
    return Status::OK();
  }

  D3D12BufferRegion dst = dml_util::CreateBufferForDeviceMemory(
      device, device_memory, size_in_bytes);

  auto byte_span = absl::Span<const uint8_t>(
      static_cast<const uint8_t*>(cpu_memory), size_in_bytes);
...
StatusOr<DmlGpuEvent> DMLDeviceContext::CopyDeviceMemoryToCPU(
    DmlDevice* device, const SP_DeviceMemoryBase* device_memory,
    void* cpu_memory, uint64_t size_in_bytes) {
  if (size_in_bytes == 0) {
    return Status::OK();
  }
...
void DMLDeviceContext::CopyMemoryInSameDevice(
    DmlDevice* device, const SP_DeviceMemoryBase* input_memory,
    const SP_DeviceMemoryBase* output_memory, uint64_t size_in_bytes) const {
  if (size_in_bytes == 0) {
    return;
  }

  D3D12BufferRegion src = dml_util::CreateBufferForDeviceMemory(
      device, input_memory, size_in_bytes);

  D3D12BufferRegion dst = dml_util::CreateBufferForDeviceMemory(
      device, output_memory, size_in_bytes);
...
DmlBuffer DMLDeviceContext::AllocateDefaultBuffer(
    OpKernelContext* op_kernel_context, uint64_t num_bytes) const {
