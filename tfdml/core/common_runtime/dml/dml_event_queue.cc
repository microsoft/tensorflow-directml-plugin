...
void DmlEventQueue::Enqueue(DmlGpuEvent gpu_event, DoneCallback done_callback) {
  ...
  assert(state->fence->GetCompletedValue() + 1 >=
  ...
  if (state->current_awaited_fence_value <= gpu_event.fence_value) {
    ...
    assert(state->events_by_fence_value.lower_bound(gpu_event.fence_value) ==
...
/*static*/ void DmlEventQueue::ThreadProc(std::shared_ptr<SharedState> state) {
  ...
  while (true) {
    ...
    // here is to ensure all outstanding events are signaled, but this queue
    // uses the *next* value (0) to flush events in a batch. If the device is
    // removed we explicitly set the next_fence_value to UINT64_MAX so that all
    // events are flushed then exit this thread.
