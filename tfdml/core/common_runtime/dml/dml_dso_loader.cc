...
std::string GetModuleDirectory() {
  ...
  CHECK(PathCchRemoveFileSpec(const_cast<wchar_t*>(wpath.data()),
                              wpath.size()) == S_OK);
...
StatusOr<void*> GetDsoHandle(const std::string& name,
  ...
  if (status.ok()) {
    TF_Log(TF_INFO, "Successfully opened dynamic library %s", filename.c_str());
    return dso_handle;
  }
...
StatusOr<void*> GetDirectMLLibraryHandle(const std::string& basename) {
  ...
  if (path.empty()) {
    ...
    // The DirectML DLL can't be located next to the pluggable device library
    // because TensorFlow would try to load it, so we place it in a "directml"
    // folder that we append to the search path
...
#else
  return Status(TF_UNIMPLEMENTED, "Kernel32.dll is only available on Windows");
#endif
}
