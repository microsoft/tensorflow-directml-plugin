...
class BinaryOpTransposer : public LayoutAgnosticOpTransposer
{
  ...
  private:
    bool IsNDOperateWithMD(const MutableNodeView& node, int n, int m);
    bool IsFaninShapeSupported(const MutableNodeView& node, int rank);
    std::vector<int> GetNDDataFaninPorts(const MutableNodeView& node, int rank);
    Status AddNodeShapeConst(
        Mutation* mutation,
        absl::string_view node_name,
    ...
    Status AddNodeReshape(
        ...
        const tensorflow::DataType& data_type);
    Status MaybeReshapeVectorFanin(
        TransposeContext* context,
...
class SqueezeTransposer : public LayoutAgnosticOpTransposer
{
  ...
  private:
    ...
    bool IsAlongAxis(
        ...
        int rank) const;
    bool IsDimsSupported(
        const TransposeContext& context,
        const MutableNodeView& node) const;
    Status UpdateSqueezeDims(TransposeContext* context, MutableNodeView* node);
};
...
class StridedSliceTransposer : public LayoutAgnosticOpTransposer
{
  ...
  private:
    bool IsMaskZero(const MutableNodeView& node, absl::string_view mask);
    bool HasOnlyBeginEndMask(const MutableNodeView& node);
    Status PermuteMask(
        TransposeContext* context,
        MutableNodeView* node,
...
class ReduceTransposer : public LayoutAgnosticOpTransposer
{
  ...
  private:
    bool KeepDims(const MutableNodeView& node);
    bool IsAlongAxis(
        const tensorflow::TensorProto& tensor,
        absl::Span<const int> axis,
        int rank);
    bool IsReduceAxisSupported(
        const TransposeContext& context,
        const MutableNodeView& node,
        int rank);
};
...
class SelectTransposer : public LayoutAgnosticOpTransposer
{
  ...
  protected:
    bool IsFaninScalarVector4D(const MutableNodeView& fanin, int port);
    std::vector<int> GetFaninPorts(const MutableNodeView& fanin, int port);
};
...
class FusedBatchNormGradTransposer : public LayoutSensitiveOpTransposer
{
  ...
  private:
    bool IsTraining(const MutableNodeView& node) const;
};
