...
#include "tfdml/core/util/allocator_retry.h"
...
class BFCAllocator : public Allocator
{
  ...
  private:
    ...
    // Chunks whose freed_at_count is later than the safe frontier value are
    // kept on a special list and not subject to merging immediately upon being
    // freed.
    //
    ...
    //
    // If required_bytes > 0 then this function is being called in the context
    // of a need for this many bytes that could not be satisfied without merging
    ...
    //
    // An AllocationRegion's memory is split up into one or more disjoint
    // Chunks, which together cover the whole region without gaps.  Chunks
    // participate in a doubly-linked list, and the prev/next pointers point to
    // the physically adjacent chunks.
    //
    // Since a chunk cannot be partially in use, we may need to split a free
    // chunk in order to service a user allocation.  We always merge adjacent
    // free chunks.
    //
    ...
    struct Chunk
    {
        ...
        // allocation_id is set to -1 when the chunk is not in use. It is
        // assigned a value greater than zero before the chunk is returned from
        ...
        // If not kInvalidChunkHandle, the memory referred to by 'prev' is
        // directly preceding the memory used by this chunk.  E.g., It should
        // start at 'ptr - prev->size'
        ChunkHandle prev = kInvalidChunkHandle;

        // If not kInvalidChunkHandle, the memory referred to by 'next' is
        // directly following the memory used by this chunk.  E.g., It should be
        // at 'ptr + size'
    ...
    struct Bin
    {
        ...
        Bin(BFCAllocator* allocator, size_t bs)
            : bin_size(bs),
              free_chunks(ChunkComparator(allocator))
        {
        }
    };
    ...
    void RemoveFreeChunkIterFromBin(
        ...
        const Bin::FreeChunkSet::iterator& c) EXCLUSIVE_LOCKS_REQUIRED(lock_);
